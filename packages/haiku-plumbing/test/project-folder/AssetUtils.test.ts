...
const { existsSync, mkdirpSync, readFileSync, writeFileSync } = fse;
...
tape("AssetUtils", suite => {
  suite.test("dumpBase64Images", test => {
    ...
    TestHelpers.tmpdir((folder, teardown) => {
      ...
      const pngImageData = Buffer.from("<some PNG image stuff>").toString(
        "base64"
      );
      const gifImageData = Buffer.from("<some GIF image stuff>").toString(
        "base64"
      );
      ...
      test.true(
        existsSync(`${abspath}.processed`),
        ".processed file was written"
      );
      test.is(
      ...
      test.is(
        readFileSync(
          join(folder, "assets", "designs", "foo_image_1.png")
        ).toString(),
      ...
      test.is(
        readFileSync(
          join(folder, "assets", "designs", "foo_image_2.gif")
        ).toString(),

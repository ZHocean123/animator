    ...
    return async.series(
      [
        cb => {
          ...
          plumbing.envoyHandlers.user
            .authenticate("jenkins@haiku.ai", "supersecure")
            .then(resultingIdentity => {
        ...
        cb => {
          const [mockCreateProject, unstub] = stubProperties(
            plumbing.envoyHandlers.project,
            "createProject"
          );
        ...
        cb =>
          plumbing.bootstrapProject(
            { ...project, skipContentCreation: true },
            err => {
              if (err) {
                return cb(err);
              }
        ...
        cb => {
          ...
          fse.outputFileSync(
            path.join(project.projectPath, "Quatro.txt"),
            "44"
          );
        ...
        cb => {
          ...
          fse.outputFileSync(
            path.join(project.projectPath, "Cinco.txt"),
            "555"
          );
          return cb();
        },
        ...
        cb => {
          ...
          fse.outputFileSync(
            path.join(project.projectPath, "Tres.txt"),
            "3333"
          );
          fse.outputFileSync(
            path.join(project.projectPath, "Quatro.txt"),
        ...
        cb => {
          fse.outputFileSync(
            path.join(project.projectPath, "Uno.txt"),
            "11111"
          );
          fse.outputFileSync(
            path.join(project.projectPath, "Dos.txt"),
            "22222"
          );
          fse.outputFileSync(
            path.join(project.projectPath, "Tres.txt"),

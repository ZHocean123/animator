...
import * as Lock from "haiku-serialization/src/bll/Lock.js";
...
export default class MasterGitProject extends EventEmitter {
  constructor(folder) {
    ...
    if (!this.folder) {
      throw new Error(
        "[master-git] MasterGitProject cannot launch without a folder defined"
      );
    }
      ...
      [
        ...
        cb => {
          return this.safeGitStatus({ log: false }, gitStatuses => {
            this.folderState.doesGitHaveChanges = !!(
              gitStatuses && Object.keys(gitStatuses).length > 0
            );
            this.folderState.isGitInitialized = fse.existsSync(
              path.join(this.folder, ".git")
            );
            return cb();
          });
        },
        ...
        cb => {
          fse.readJson(
            path.join(this.folder, "package.json"),
            (err, packageJsonObj) => {
  ...
  hasAnyPendingCommits() {
    return (
      this._isCommittingLocked || this.getPendingCommitRequests().length > 0
    );
  }
  ...
  bumpSemverAppropriately(cb) {
    ...
    return Git.listTags(this.folder, (err, tags) => {
      ...
      const pkgTag = fse.readJsonSync(path.join(this.folder, "package.json"))
        .version;
  ...
  makeTag(cb) {
    logger.info(
      `[master-git] git tagging: ${this.folderState.semverVersion} (commit: ${this.folderState.commitId})`
    );
    ...
    if (this.folderState.semverTagAttempts > MAX_SEMVER_TAG_ATTEMPTS) {
      return cb(
        new Error("Failed to make semver tag even after many attempts")
      );
    }

    return Git.createTag(
      this.folder,
      this.folderState.semverVersion,
      this.folderState.commitId,
      this.folderState.semverVersion,
      err => {
        if (err) {
          ...
          if (err.message && err.message.match(/Tag already exists/i)) {
            logger.info(
              `[master-git] git tag ${this.folderState.semverVersion} already exists; trying to bump it`
            );
        ...
        logger.info(
          `[master-git] git tagged: ${this.folderState.semverVersion}`
        );
  ...
  retryCloudSaveSetup(cb) {
    logger.info(
      `[master-git] retrying remote ref setup to see if we can cloud save after all`
    );
  ...
  pushToRemote(cb) {
    if (this.folderState.saveOptions && this.folderState.saveOptions.dontPush) {
      logger.info(
        "[master-git] skipping push to remote, per your saveOptions flag"
      );
  ...
  moveContentsToTemp(cb) {
    ...
    return tmp.dir({ unsafeCleanup: true }, (err, tmpDir, tmpDirCleanupFn) => {
      ...
      if (this.folderState.folderEntries.length < 1) {
        logger.info(
          "[master-git] folder had no initial content; skipping temp folder step"
        );
      ...
      logger.info(
        "[master-git] copying contents from",
        this.folder,
        "to temp dir",
        this.folderState.tmpDir
      );
      ...
      return fse.copy(this.folder, this.folderState.tmpDir, err => {
        ...
        try {
          ...
          logger.info(
            "[master-git] could not remove .git folder from temp dir",
            exception
          );
        }
  ...
  ensureLocalRemote(cb) {
    ...
    return Git.upsertRemoteDirectly(
      this.folder,
      this.folderState.projectName,
      repositoryUrl,
      cb
  ...
  ensureBranch(cb) {
    return Git.open(this.folder, (err, repository) => {
      ...
      return this.safeSetupBranch(
        repository,
        this.folderState.branchName,
        this.folderState.headCommitId,
        cb
      );
    });
  }
  ...
  safeSetupBranch(repository, branchName = DEFAULT_BRANCH_NAME, commitId, cb) {
    ...
    return repository.createBranch(branchName, commitId.toString()).then(
      ...
      branchErr => {
        ...
        if (
          branchErr.message &&
          branchErr.message.match(/reference with that name already exists/) &&
          branchErr.message.split(refSpec).length > 1
        ) {
          logger.info(
            "[master-git] remote refs: branch already exists; proceeding"
          );
    ...
    return Git.open(this.folder, (err, repository) => {
      ...
      return fse.outputFile(path.join(this.folder, "README.md"), "", err => {
        ...
        return Git.addAllPathsToIndex(this.folder, (err, oid) => {
          ...
          return Git.buildCommit(
            this.folder,
            this.folderState.authorName,
            null,
            `Base commit (via Haiku)`,
            oid,
            null,
            null,
            (err, commitId) => {
                ...
                (err, branchName) => {
                  ...
                  return Git.lookupRemote(
                    ...
                    (err, mainRemote) => {
                      ...
                      logger.info(
                        "[master-git] remote refs: pushing refspecs",
                        remoteRefspecs,
                        "over https"
                      );

                      return mainRemote
                        .push(remoteRefspecs, Git.globalPushOpts)
                        .then(() => {
                          return cb();
                        }, cb);
                    }
  ...
  copyContentsFromTemp(cb) {
    ...
    if (!this.folderState.tmpDir) {
      logger.info(
        "[master-git] no temp dir seems to have been created at",
        this.folderState.tmpDir
      );
      return cb();
    }
  ...
  pullRemotePostFetch(cb) {
    return Git.getCurrentBranchName(this.folder, (err, partialBranchName) => {
      ...
      const remoteBranchRefName = Git.getRemoteBranchRefName(
        this.folderState.projectName,
        partialBranchName
      );
      return Git.getReference(this.folder, remoteBranchRefName, (err, ref) => {
        ...
        if (!ref) {
          logger.info(
            `[master-git] skipping merge after pull since no ref ${remoteBranchRefName} exists`
          );
        ...
        return Git.mergeProject(
          this.folder,
          this.folderState.projectName,
          partialBranchName,
          this.folderState.saveOptions,
          (err, didHaveConflicts, shaOrIndex) => {
            ...
            return this.safeGitStatus({ log: true }, () => {
              ...
              this.folderState.mergeCommitId = didHaveConflicts
                ? null
                : shaOrIndex.toString();
              return cb();
            });
  ...
  pullRemote(cb) {
    ...
    return Git.fetchProjectDirectly(
      this.folder,
      this.folderState.projectName,
      repositoryUrl,
      err => {
        if (err) {
          ...
          logger.info(
            `[master-git] unable to fetch because ${err}; ignoring this`
          );
  ...
  conflictResetOrContinue(cb) {
    ...
    return this.safeGitStatus({ log: true }, () => {
      return Git.cleanAllChanges(this.folder, err => {
        ...
        return Git.hardResetFromSHA(
          this.folder,
          this.folderState.commitId.toString(),
          err => {
            if (err) {
              return cb(err);
            }
  ...
  doesGitHaveChanges(cb) {
    return this.fetchFolderState("does-git-have-changes", {}, () => {
      ...
      if (this.folderState.doesGitHaveChanges) {
        logger.info(
          "[master-git] looks like git has changes; must do full save"
        );
        return cb(null, true);
      }
  ...
  cloudSaveDisabled(cb) {
    const error = new Error(
      "Project was saved locally, but could not sync to Haiku Cloud"
    );
    error.code = 1;
    return cb(error);
  }
  ...
  pushTagDirectly(cb) {
    logger.info(
      `[master-git] pushing tag ${this.folderState.semverVersion} to remote (${this.folderState.projectName})`
    );
    return Git.pushTagToRemoteDirectly(
      this.folder,
      this.folderState.projectName,
      this.folderState.semverVersion,
      cb
  ...
  commitActual(commitOptions, cb) {
    ...
    return this.fetchFolderState("commit-project", {}, () => {
      return Git.commitProject(
        this.folder,
        this.folderState.authorName,
        this.folderState.hasHeadCommit,
        finalOptions,
        addable,
        (err, commitId) => {
          if (err) {
        ...
        cb => {
          return this.fetchFolderState(
            ...
            err => {
              ...
              logger.info(
                "[master-git] folder initialization status:",
                this.folderState
              );
              return cb();
            }
    ...
    return async.series(
      [
        ...
        cb => {
          ...
          if (
            !(
              this._projectInfo.projectName &&
              this.folderState.remoteProjectDescriptor
            )
          ) {
          ...
          const teardownSteps = ["commitEverything", "makeTag"];
          ...
          logger.info(
            "[master-git] project save: action sequence:",
            actionSequence
          );

...
import * as Illustrator from 'haiku-serializationbll/Illustrator'
import * as Project from 'haiku-serializationbll/Project'
import * as Sketch from 'haiku-serializationbll/Sketch'
import * as EmitterManager from 'haiku-serializationutils/EmitterManager'
import * as logger from 'haiku-serialization'
import * as walkFiles from 'haiku-serializationutils/walkFiles'
...
function isFileSignificant(relpath) {
  return !UNWATCHABLE_RELPATHS[relpath]
    && !UNWATCHABLE_BASENAMES[path.basename(relpath)]
    && WATCHABLE_EXTNAMES[path.extname(relpath).toLowerCase()]
}

function doSkipFile(relpath) {
  return GITIGNORED_BASENAMES[relpath]
...
export default class Master extends EventEmitter {
  ...
  unlinkAsset(relpath, cb) {
    ...
    Object.keys(this._knownLibraryAssets)
      .filter(path => path.includes(relpath))
...
function changeToCommitMessage(relpath, { method, params, metadata, timestamp }) {

...
import { storeConfigValues } from "./project-folder/ProjectDefinitions.js";
...
export const HAIKU_WS_SECURITY_TOKEN =
  Math.random()
    .toString(36)
    .substring(7) +
  Math.random()
...
const PINFO = `${process.pid} ${path.basename(__filename)} ${path.basename(
  process.execPath
)}`;
...
process.on("exit", code => {
  logger.info(
    `[plumbing] plumbing process (${PINFO}) exiting with code ${code}`
  );
  teardownPlumbings(() => {});
});
process.on("SIGINT", () => {
...
export default class Plumbing extends EventEmitter {
  ...
  launch(haiku = {}, cb) {
    ...
    return this.envoyServer.ready().then(() => {
      ...
      this.envoyServer.bindHandler(
        ERROR_CHANNEL,
        ErrorHandler,
        this.envoyHandlers.error
      );
      this.envoyServer.bindHandler(
        TIMELINE_CHANNEL,
        TimelineHandler,
        this.envoyHandlers.timeline
      );
      this.envoyServer.bindHandler(
        TOUR_CHANNEL,
        TourHandler,
        this.envoyHandlers.tour
      );
      this.envoyServer.bindHandler(
        EXPORTER_CHANNEL,
        ExporterHandler,
      ...
      this.envoyServer.bindHandler(
        USER_CHANNEL,
        UserHandler,
        this.envoyHandlers.user
      );
      this.envoyServer.bindHandler(
        GLASS_CHANNEL,
        GlassHandler,
        this.envoyHandlers.glass
      );
      this.envoyServer.bindHandler(
        PROJECT_CHANNEL,
        ProjectHandler,
      ...
      this.envoyServer.bindHandler(
        SERVICES_CHANNEL,
        ServicesHandler,
        this.envoyHandlers.services
      );

      this.envoyHandlers.user.on(
        `${USER_CHANNEL}:load`,
      ...
      return this.launchControlServer(
        haiku.socket,
        haiku.envoy.host,
        (err, server, host, port) => {
          ...
          server.on("connection", (websocket, request) => {
            ...
            if (haiku.socket.token && params.token !== haiku.socket.token) {
              logger.info(
                `[plumbing] websocket connected with bad token ${params.token}`
              );
            ...
            logger.info(
              `[plumbing] websocket for ${folder ||
                "main"} connected (${type} ${alias})`
            );
            ...
            for (let i = this.clients.length - 1; i >= 0; i--) {
              ...
              if (client.params) {
                if (
                  client.params.alias === alias &&
                  client.params.folder === folder
                ) {
  ...
  handleRemoteMessage(type, alias, folder, message, cb) {
    ...
    if (message.type === "relay") {
      return this.relayMessage(folder, message);
    }
  ...
  methodMessageAfterLog(message, err, result, alias) {
    if (!IGNORED_METHOD_MESSAGES[message.method]) {
      if ((err && err.message) || (err && err.stack)) {
        logger.info(
          `[plumbing] ${message.method} error ${err.stack || err.message}`
        );
      }
      logger.info(`[plumbing] ↑-- ${message.method} via ${alias} --↑`);
    }
  ...
  executeMethodMessagesWorker() {
    ...
    if (message.type === "action") {
      return this.handleClientAction(
        type,
        alias,
        folder,
        message.method,
  ...
  plumbingMethod(method, params = [], cb) {
    ...
    return this[method].apply(
      this,
      params.concat((error, result) => {
        if (error) {
          return cb(error);
        }
  ...
  awaitClientWithQuery(query, timeout, cb) {
    ...
    if (timeout <= 0) {
      logger.warn(
        `[plumbing] timed out waiting for client ${JSON.stringify(fixed)}`
      );
      return cb(new Error("E_TIMEOUT"));
    }

    const clientMatching = find(this.clients, { params: fixed });
  ...
  masterHeartbeat(folder, cb) {
    return this.awaitMasterAndCallMethod(
      folder,
      "masterHeartbeat",
      [{ from: "master" }],
      cb
  ...
  bootstrapProject(project, finish) {
    ...
    return async.series(
      ...
      err => {
        ...
        return this.initializeFolder(project, err => {
          if (err) {
            return finish(err);
          }
  ...
  duplicateProject(destinationProject, sourceProject, cb) {
    if (!sourceProject.projectExistsLocally) {
      logger.info(
        `[plumbing] source project did not exist during duplicate: ${sourceProject.projectName}`
      );
    ...
    if (destinationProject.projectExistsLocally) {
      ...
      // as a result.
      logger.warn(
        `[plumbing] source project existed locally during duplicate: ${destinationProject.projectName}`
      );
    }
  ...
  saveProject(project, saveOptions, cb) {
    ...
    return this.awaitMasterAndCallMethod(
      project.projectPath,
      "saveProject",
      [project, saveOptions, { from: "master" }],
      cb
    );
  }
  ...
  handleClientAction(type, alias, folder, method, params, cb) {
    ...
    return async.eachSeries(
      [Q_GLASS, Q_TIMELINE, Q_CREATOR, Q_MASTER],
      (clientSpec, nextStep) => {
        ...
        this.sendQueriedClientMethod(
          assign({ folder }, clientSpec),
          method,
          params,
          () => {}
        );
...
function logActionInitiation(method, clientSpec) {
  if (!IGNORED_METHOD_MESSAGES[method]) {
    logger.info(
      `[plumbing] -> client action ${method} being sent to ${clientSpec.alias}`
    );
  }
}
...
function logAndHandleActionResult(err, cb, method, type, alias) {
  if (!IGNORED_METHOD_MESSAGES[method]) {
    ...
    logger.info(
      `[plumbing] <- client action ${method} from ${type}@${alias} ${status}`,
      err
    );
  }
...
Plumbing.prototype.awaitMasterAndCallMethod = function(
  folder,
  method,
  params,
  cb
  ...
  if (!master) {
    return setTimeout(
      () => this.awaitMasterAndCallMethod(folder, method, params, cb),
      AWAIT_INTERVAL
    );
  }
...
Plumbing.prototype.upsertMaster = function({
  folder,
  fileOptions,
  envoyOptions,
  envoyHandlers
  ...
  if (!this.masters[folder]) {
    const master = new Master(folder, fileOptions, envoyOptions, envoyHandlers);
...
Plumbing.prototype.launchControlServer = function launchControlServer(
  socketInfo,
  host,
  cb
) {
  if (socketInfo && socketInfo.port) {
    logger.info(
      `[plumbing] plumbing websocket server listening on specified port ${socketInfo.port}...`
    );
...
Plumbing.prototype.extendEnvironment = function extendEnvironment(haiku) {
  ...
  logger.info(
    "[plumbing] environment forwarding:",
    JSON.stringify(HAIKU_ENV, 2, null)
  );
...
Plumbing.prototype.createControlSocket = function createControlSocket(
  socketInfo
) {

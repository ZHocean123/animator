import { get, to } from "@haiku/core/lib/vendor/color-string/index.js";
...
export function derivateStringFromColorResult(
  result: ColorResult & { source: string }
) {
  const values: [number, number, number, number] =
    Number(result.source) === DisplayValues.HSL
      ? [
          roundValue(result.hsl.h),
          roundValue(result.hsl.s * 100),
          roundValue(result.hsl.l * 100),
          result.hsl.a
        ]

...
const cli = new Nib({
  ...
  version: pkg.version,
  description:
    "The Haiku Admin CLI — Enables Haiku administrative access and actions from the command line.",
  ...
  commands: [
    {
      name: "user-login",
      description:
        "Allows impersonation of a user, by getting a valid auth token for that user from inkstone.  " +
    ...
    (err, authResponse, httpResponse) => {
      if (err !== undefined) {
        if (httpResponse.statusCode === 401) {
          context.writeLine(
            chalk.bold.red(
              "You must be logged in as an admin to do that.  (haiku login)"
            )
          );
        } else {

...
export class TourHandler implements Tour {
  ...
  private requestShowStep(
    state: TourState,
    position: ClientBoundingRect
  ): void {
    this.server.emit(TOUR_CHANNEL, {
      payload: {
        ...state,
        ...
        stepData: {
          ...
          total: this.platformStates.length - 1
        }
      },
  ...
  receiveWebviewCoordinates(
    webview: string,
    coordinates: ClientBoundingRect
  ): void {
  ...
  start(force?: boolean): void {
    if ((!didTakeTour() && !this.isActive) || force) {
      this.currentStep = 0;
      this.isActive = true;
      this.requestShowStep(
        { ...this.states[this.currentStep] },
        { top: "40%", left: "50%", width: 0, height: 0 }
      );
    }
  }

...
export function crashReportFork(projectPath: string, zipName: string, zipPath: string, finalUrl: string) {
...
function ensureFolderExist(folder: string): Promise<void> {
...
function getPreSignedURL(zipName: string): Promise<string> {
...
function upload(url: string, filePath: string): Promise<request.RequestResponse> {
  return new Promise((resolve, reject) => {
...
function zipProjectFolders(destination: string, sources: string[]) {
...
export function crashReportCreate(cb: () => void) {
  ...
  ensureFolderExist(HOMEDIR_PROJECTS_PATH)
    ...
    .then(() => zipProjectFolders(
      process.env.HAIKU_CRASH_REPORT_ZIP_PATH,
      [process.env.HAIKU_CRASH_REPORT_PROJECT_PATH, HOMEDIR_LOGS_PATH],
    ))

...
import {
  ...
  // @ts-ignore
} from "haiku-serialization/src/utils/HaikuHomeDir.js";
// @ts-ignore
...
export const crashReportFork = (
  ...
  logger.info(
    "[carbonite] initiating crash report",
    projectPath,
    zipName,
    zipPath,
    cleanPresignedUrl(finalUrl)
  );
...
const upload = (
  ...
  return new Promise((resolve, reject) => {
    readFile(filePath, (err, data) => {
      ...
      logger.info(
        `[carbonite] uploading ${filePath} to ${cleanPresignedUrl(url)}`
      );
...
export const crashReportCreate = (cb: () => void): void => {
  logger.info(
    `[carbonite] preparing crash report for ${cleanPresignedUrl(
      process.env.HAIKU_CRASH_REPORT_URL
    )}`
  );
  ...
  ensureFolderExist(HOMEDIR_PROJECTS_PATH)
    ...
    .then(() =>
      zipProjectFolders(process.env.HAIKU_CRASH_REPORT_ZIP_PATH, [
        process.env.HAIKU_CRASH_REPORT_PROJECT_PATH,
        HOMEDIR_LOGS_PATH
      ])
    )

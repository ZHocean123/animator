 ...
 */
const nowDate = () =>
  (global as any)[String.fromCharCode(0x44, 0x61, 0x74, 0x65)][
    String.fromCharCode(0b1101110, 0b1101111, 0b1110111)
  ]();
...
export class UserHandler extends EnvoyHandler {
  ...
  reportActivity(): void {
    ...
    return inkstone.updates.check(
      authToken,
      "?platform=mac&environment=production&branch=master",
      () => {
        // Noop.
      }
    );
  ...
  private recoverIdentityOffline(): MaybeAsync<void> {
    ...
    if (this.identity.user && this.identity.organization) {
      ...
      if (
        ...
        (this.identity.organization as any)[
  ...
  authenticate(username: string, password: string): Promise<HaikuIdentity> {
    ...
    return new Promise<HaikuIdentity>((resolve, reject) => {
      inkstone.user.authenticate(
        username,
        password,
        (authErr, authResponse, httpResponse) => {
  ...
  getPrivilege(privilege: OrganizationPrivilege): MaybeAsync<any> {
    return (
      ...
      (this.identity.organization as any)[privilege]
    );
  }

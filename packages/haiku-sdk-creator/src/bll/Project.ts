...
import { requestInstance } from "@haiku/sdk-inkstone/lib/transport.mjs";
import fse from "fs-extra";
const { existsSync, move, readFile } = fse;
// @ts-ignore
import { HOMEDIR_PROJECTS_PATH } from "haiku-serialization/src/utils/HaikuHomeDir.js";
...
export class ProjectHandler extends EnvoyHandler {
  ...
  constructor(
    ...
    this.userHandler.on(`${USER_CHANNEL}:load`, ({ organization }) => {
      this.registry = new Registry(
        path.join(FILE_PATHS.HAIKU_HOME, "projects", organization.Name)
      );
    });
  }

  private inkstoneProjectToHaikuProject(
    project: inkstone.project.Project
  ): HaikuProject {
    const {
      organization,
      user
    } = this.userHandler.getIdentity() as HaikuIdentity;
    const organizationName = getSafeOrganizationName(
  ...
  private retrieveProjectsList(): HaikuProject[] {
    return (this.getConfig<HaikuProject[]>(ProjectSettings.List) || []).map(
      project => ({
        ...project,
  ...
  private getMergedProjectList(inboundRemoteList: HaikuProject[]) {
    ...
    return this.retrieveProjectsList()
      .filter(
        project =>
          project.local &&
          !inboundRemoteList.find(
            remoteProject =>
              remoteProject.projectName.toLowerCase() ===
      ...
      inkstone.project.list((error, projects) => {
        if (error) {
          this.server.logger.warn(
            "[haiku envoy server] error while listing projects"
          );
          if (error.message === ErrorCode.ErrorCodeAuthorizationRequired) {
            return reject({ code: ProjectError.Unauthorized });
          }
          if (
            this.userHandler.getPrivilege(
              OrganizationPrivilege.EnableOfflineFeatures
            )
          ) {
        ...
        const list = this.getMergedProjectList(
          projects.map(project => this.inkstoneProjectToHaikuProject(project))
        );
  ...
  private archiveProject(haikuProject: HaikuProject, resolve: () => void) {
    if (existsSync(haikuProject.projectPath)) {
      ...
      if (existsSync(archivePath)) {
        let i = 0;
        while (
          existsSync((archivePath = `${haikuProject.projectPath}.bak.${i++}`))
        ) {
          // ...
        }
      }
  ...
  private deleteProjectOffline(
    haikuProject: HaikuProject,
    resolve: () => void
  ) {
    // "Remove" the project without inkstone.
    this.setConfig<HaikuProject[]>(
      ProjectSettings.List,
      this.retrieveProjectsList().filter(
        project => project.projectName !== haikuProject.projectName
      )
    );
  ...
  deleteProject(haikuProject: HaikuProject): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      inkstone.project.deleteByName(
        { Name: haikuProject.projectName },
        deleteErr => {
          if (deleteErr) {
            return reject({ code: ProjectError.Offline });
          }
          ...
          this.getProjectsList().then(() => {
            this.server.logger.info(
              "[haiku envoy server] reloaded projects list"
            );
          });
  ...
  createProject(
    name: string,
    allowOffline = true,
    deferCaudexBacking = true
  ): Promise<HaikuProject> {
  ...
  forkProject(
    ...
    this.server.logger.info(
      "[haiku envoy server] forking project",
      organizationName,
      projectName
    );
    ...
    return new Promise((resolve, reject) => {
      inkstone.community.forkCommunityProject(
        communityProject,
        (error, forkedProject) => {
          if (error) {
            return reject(error);
  ...
  updateProject(
    haikuProject: HaikuProject,
    ensureCaudexBacking = false
  ): Promise<HaikuProject> {
    return new Promise((resolve, reject) => {
      inkstone.project.update(
        {
          Name: haikuProject.projectName,
          IsPublic: haikuProject.isPublic,
          EnsureCaudexBacking: ensureCaudexBacking
        },
        (error, project) => {
          if (error) {
            return reject(error);
          ...
          Object.assign(
            haikuProject,
            this.inkstoneProjectToHaikuProject(project)
          );
  ...
  getExporterAssetRequests(
    project: HaikuProject
  ): MaybeAsync<ExporterRequest[]> {

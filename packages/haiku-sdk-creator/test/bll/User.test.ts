...
tape("User", async suite => {
  ...
  suite.test("authenticate.noResponse", test => {
    const [mockAuthenticate, unstub] = stubProperties(
      inkstone.user,
      "authenticate"
    );
  ...
  suite.test("authenticate.unauthorized", test => {
    ...
    userHandler.authenticate("foo", "bar").catch(error => {
      test.deepEqual(
        ...
        '401 is forwarded with message "Unauthorized"'
      );
      unstub();
      test.end();
    });
  ...
  suite.test("authenticate.500", test => {
    ...
    userHandler.authenticate("foo", "bar").catch(error => {
      test.deepEqual(
        error,
        { code: 500, message: "Auth HTTP Error: 400" },
        "generic errors resolve to code 500"
      );
      unstub();
      test.end();
    });
  ...
  suite.test("authenticate.e2e", test => {
    const [mockSetConfig, unstubInkstone] = stubProperties(
      inkstone,
      "setConfig"
    );
    const [mockList, unstubAuthenticate] = stubProperties(
      inkstone.organization,
      "list"
    );
    const [mockAuthenticate, mockGet, unstubOrg] = stubProperties(
      inkstone.user,
      "authenticate",
      "get"
    );
    ...
    const [mockSetConfigObfuscated, unstub] = stubProperties(
      userHandler,
      "setConfigObfuscated"
    );
    mockAuthenticate.callsArgWith(
      2,
      undefined,
      { Token: "auth_token" },
      { statusCode: 200 }
    ...
    userHandler.authenticate("foo", "bar").then(identity => {
      test.true(
        mockAuthenticate.calledWith("foo", "bar"),
        "inkstone.user.authenticate called with credentials"
      );
      test.true(
        mockSetAuthToken.calledWith("auth_token"),
        "sdkClient receives auth token for persistence"
      );
      test.true(
        mockSetConfig.calledWithMatch({ authToken: "auth_token" }),
        "auth token was set in inkstone during load"
      );
      test.deepEqual(
        identity.user,
        { Username: "foo@foo.com" },
        "user was stored in identity"
      );
      test.deepEqual(
        identity.organization,
        { Name: "Foo" },
        "org was stored in identity"
      );

...
class StageTitleBar extends React.Component {
  constructor(props) {
    ...
    this.handleConglomerateComponent = this.handleConglomerateComponent.bind(
      this
    );
    this.handleShowProjectLocationToast = this.handleShowProjectLocationToast.bind(
      this
    );
  ...
  componentDidMount() {
    ...
    this._fetchMasterStateInterval = setInterval(() => {
      if (this.props.projectModel) {
        return this.props.projectModel.masterHeartbeat(
          (heartbeatErr, masterState) => {
            if (heartbeatErr || !masterState) {
              ...
              if (!heartbeatErr) {
                ...
                heartbeatErr = new Error(
                  "Unknown problem with master heartbeat"
                );
              }
              ...
              return this.props.createNotice({
                ...
                title: "Uh oh!",
                message:
                  "Animator is having a problem accessing your project. 😢  Please restart Animator. If you see this error again, contact Haiku for support."
              });
            }
  ...
  getProjectSaveOptions() {
    return {
      saveStrategy:
        SNAPSHOT_SAVE_RESOLUTION_STRATEGIES[
          this.state.snapshotSaveResolutionStrategyName
        ]
    };
  }
  ...
  showGenericPublishError() {
    this.props.createNotice({
      ...
      title: "Uh oh!",
      message:
        "We were unable to publish your project. 😢 Please make sure you are connected to the internet."
    });
  }
  ...
  showSharePopover(cb) {
    this.setState(
      ...
      () => {
        this.props.projectModel.saveProject(
          this.props.project,
          this.getProjectSaveOptions(),
          cb
        );
      }
    );
    ...
    if (this.props.projectModel) {
      ...
      this.props.envoyProject
        ...
        .then(list => {
          this.setState(
            {
              privateProjectCount: list.filter(project => !project.isPublic)
                .length
            },
            () => {
              ...
              // be permitted.
              if (
                typeof this.props.privateProjectLimit === "number" &&
                !this.props.project.isPublic
              ) {
                this.props.envoyProject
                  .updateProject(this.props.project)
                  .then(() => this.showSharePopover(cb))
                  .catch(error => {
  ...
  performSyndicationCheck() {
    ...
    this.props.envoyProject
      .getSnapshotInfo()
      .then(({ snapshotSyndicated, shareUrls }) => {
      ...
      .catch(() => {
        this.props.createNotice({
          ...
          title: "Uh oh!",
          message:
            "We were unable to publish your project. 😢 Please try again in a bit. If you see this error again, contact Haiku for support."
        });
        this.clearSyndicationChecks();
  ...
  performProjectSave() {
    ...
    return this.requestSaveProject((snapshotSaveError, snapshotData) => {
          ...
          default:
            ...
            if (snapshotSaveError.conflicts) {
              ...
              this.props.createNotice({
                ...
                message:
                  "We couldn't save your changes. 😢 Please contact Haiku Support."
              });
              return this.setState({
                showSharePopover: false
              });
          ...
          if (this.props.envoyExporter) {
            this.abortSyndicationCheck = () => {
              ...
              this.props.createNotice({
                ...
                title: "Uh oh!",
                message:
                  "Not all assets were published successfully. 😢  If you see this error again, contact Haiku for support."
              });
            };

            this.props.envoyExporter.on(
              `${EXPORTER_CHANNEL}:abort`,
              this.abortSyndicationCheck
            );
          }
      ...
      return setTimeout(
        () => this.setState({ snapshotSaveConfirmed: false }),
        2000
      );
    });
  }
  ...
  renderSnapshotSaveInnerButton() {
    if (this.state.snapshotSaveError) {
      return (
        <div style={{ height: 18, marginRight: -5 }}>
          <DangerIconSVG fill="transparent" />
        </div>
      );
    }
  ...
  fetchProxyElementForSelection() {
    ...
    if (component) {
      return ElementSelectionProxy.fromSelection(
        Element.where({ component, _isSelected: true }),
        component
      );
    }
  }
  ...
  isEventHandlersEditorAvailable() {
    ...
    return (
      proxy &&
      (proxy.doesManageSingleElement() || proxy.hasNothingInSelection())
    );
  }
  ...
  getProxySelectionElement() {
    ...
    if (!element) {
      element = this.getActiveComponent()
        .getArtboard()
        .getElement();
    }
  ...
  get conglomerateComponentButton() {
    return (
      <button
        ...
        style={[BTN_STYLES.btnIcon, BTN_STYLES.leftBtns]}
      >
      ...
      <div style={STYLES.frame} className="frame">
            ...
            <button
              ...
              style={[
                ...
                BTN_STYLES.centerBtns,
                {
                  ...
                  ":hover": {
                    backgroundColor: Color(Palette.DARKEST_COAL).darken(0.3)
                  }
                }
              ]}
            >
        ...
        <button
          ...
          style={[
            ...
            isPreviewMode(this.props.interactionMode) && {

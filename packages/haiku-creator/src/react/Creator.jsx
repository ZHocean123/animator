import { shell, ipcRenderer, clipboard, webFrame } from "electron";
...
import EnvoyClient from "haiku-sdk-creator/src/envoy/EnvoyClient.ts";
import {
  EXPORTER_CHANNEL,
  ExporterFormat
...
import { SERVICES_CHANNEL } from "haiku-sdk-creator/lib/services/index.mjs";
import {
  ERROR_CHANNEL,
  isUserlandCulprit
...
} from "haiku-common/lib/proxies/index.mjs";
...
import {
  getAccountUrl,
  shouldEmitErrors
} from "haiku-common/lib/environments/index.mjs";
...
export default class Creator extends React.Component {
  constructor(props) {
    ...
    this.handleFindElementCoordinates = this.handleFindElementCoordinates.bind(
      this
    );
    this.handleFindWebviewCoordinates = this.handleFindWebviewCoordinates.bind(
      this
    );
    ...
    this.setGlassInteractionToPreviewMode = this.setGlassInteractionToPreviewMode.bind(
      this
    );
    this.setGlassInteractionToEditMode = this.setGlassInteractionToEditMode.bind(
      this
    );
    this.setGlassInteractionToCodeEditorMode = this.setGlassInteractionToCodeEditorMode.bind(
      this
    );
    this.togglePreviewMode = this.togglePreviewMode.bind(this);
    this.clearAuth = this.clearAuth.bind(this);
    this.tryToChangeCurrentActiveComponent = this.tryToChangeCurrentActiveComponent.bind(
      this
    );
    ...
    this.handleShowEventHandlersEditor = this.handleShowEventHandlersEditor.bind(
      this
    );
    this.handleShowConfirmGroupPopup = this.handleShowConfirmGroupPopup.bind(
      this
    );
    this.hideConfirmGroupUngroupPopup = this.hideConfirmGroupUngroupPopup.bind(
      this
    );
    this.layout = new EventEmitter();
    this.activityMonitor = new ActivityMonitor(
      window,
      this.onActivityReport.bind(this)
    );
    ...
    document.addEventListener("selectionchange", () => {
      ...
      if (
        ...
        selection.anchorNode.className === "react-draggable"
      ) {
            ...
            this.error.clearLastUploadTime().then(() => {
              ...
              if (finalUrl) {
                this.createNotice({
                  ...
                  type: "info",
                  message:
                    "Project snapshot processing. Check console log for URL."
                });
                ...
                this.createNotice({
                  ...
                  type: "warning",
                  message:
                    "We were not able to freeze your project. Check logs for details."
                });
              }
            });
          }
  ...
  isTextInputFocused() {
    ...
    return tagName && (tagName === "input" || tagName === "textarea");
  }
  ...
  getActiveComponent() {
    return (
      this.state.projectModel &&
      this.state.projectModel.getCurrentActiveComponent()
    );
  }
  ...
  openFinder() {
    if (this.state.projectModel) {
      try {
        logger.info(
          "[creator] finder opening",
          shell.openPath(this.state.projectModel.getFolder())
        );
      } catch (exception) {
  ...
  openTerminal() {
    if (this.state.projectModel) {
      try {
        logger.info(
          "[creator] terminal opening",
          opn(this.state.projectModel.getFolder(), { app: "terminal" })
        );
      } catch (exception) {
        logger.error(exception);
      }
  ...
  openFileInTextEditor(abspath) {
    ...
    try {
      logger.info(
        `[creator] editor ${editorEnv || "?"}->${editorApp} opening`,
        opn(abspath, { app: editorApp })
      );
    } catch (exception) {
      logger.error(exception);
    }
  ...
  handleEnvoyUserReady() {
      ...
      this.setState(
        ...
        () => {
          if (
            ...
            typeof this._postAuthCallback === "function"
          ) {
            ...
            const handleFailure = launchError => {
              if (launchError) {
                ...
                return this.createNotice({
                  ...
                  title: "Oh no!",
                  message:
                    "We were unable to open the folder. 😢 Please close and reopen the application and try again. If you still see this message, contact Haiku for support.",
                  closeText: "Okay",
                  lightScheme: true
                });
              }
    ...
    this.user
      .getConfig(UserSettings.LastViewedChangelog)
      .then(changelogVersion => {
        ...
        if (!lastViewedChangelog) {
          ...
          this.user.setConfig(
            UserSettings.LastViewedChangelog,
            lastViewedChangelog
          );
        }
  ...
  componentDidMount() {
    ...
    this.envoyClient.get(EXPORTER_CHANNEL).then(exporterChannel => {
      ...
      ipcRenderer.on("global-menu:save-as", (_, extension, request) => {
        ...
        dialog.showSaveDialog(
          undefined,
          {
            defaultPath: this.state.projectObject
              ? `*/${this.state.projectObject.projectName}`
              : null,
            filters: [
              {
                name: request.format,
    ...
    this.envoyClient.get(USER_CHANNEL).then(user => {
      this.user = user;
      this.handleEnvoyUserReady();
    });

    // Use a lengthy timeout for project channel, which does some CRUD.
    this.envoyClient.get(PROJECT_CHANNEL).then(project => {
      this.envoyProject = project;
      project.on(`${PROJECT_CHANNEL}:saved`, projectObject => {
        if (
          this.state.projectObject &&
          this.state.projectObject.projectName === projectObject.projectName
        ) {
          this.setState({
            projectObject,
            projectFolder: projectObject.projectPath
          });
        }
      });
    });
    ...
    this.envoyClient
      .get(SERVICES_CHANNEL, { timeout: FIGMA_IMPORT_TIMEOUT })
      .then(servicesEnvoyClient => {
    ...
    this.envoyClient.get(TOUR_CHANNEL).then(tourChannel => {
      ...
      tourChannel.on(
        "tour:requestElementCoordinates",
        this.handleFindElementCoordinates
      );

      tourChannel.on(
        "tour:requestWebviewCoordinates",
        this.handleFindWebviewCoordinates
      );

      tourChannel.on(
        "tour:requestShowStep",
  ...
  handleConnectedProjectModelStateChange({ from, folder, what, value }) {
    ...
    this._projectStates[folder][what][from] =
      value !== undefined
  ...
  setInteractionMode(interactionMode) {
    if (this.state.projectModel) {
      this.state.projectModel.setInteractionMode(
        interactionMode,
        { from: "creator", integrity: false },
        () => {}
      );
    }
  }
  ...
  authenticateUser(username, password, cb) {
    ...
    this.user
      ...
      .then(({ user, organization }) => {
        mixpanel.haikuTrack("creator:user-authenticated", {
          username: user.Username
        });
  ...
  resendEmailConfirmation(username) {
    return this.props.websocket.request(
      { method: "resendEmailConfirmation", params: [username] },
      () => {}
    );
  }
    ...
    this.setState(
      ...
      () => {
        this.createNotice({
          ...
          title: "Oh no!",
          message:
            "We couldn't open this project. 😩 Please ensure that your computer is connected to the Internet. If you're connected and you still see this message your files might still be processing. Please try again in a few moments. If you still see this error, contact Haiku for support.",
          closeText: "Okay",
          lightScheme: true
        });
      }
    ...
    this.envoyProject
      ...
      .catch(error => {
        ...
        this.createNotice({
          ...
          title: "Oh no!",
          message:
            "We couldn't create your project. 😩 If this problem occurs again, please contact Haiku support.",
          closeText: "Okay",
          lightScheme: true
        });
        callback(error);
  ...
  launchProject(projectObject, cb) {
    ...
    return this.props.websocket.request(
      { method: "bootstrapProject", params: [projectObject] },
      err => {
        if (err) {
        ...
        return this.props.websocket.request(
          { method: "startProject", params: [projectObject] },
          (startProjectError, applicationImage) => {
              ...
              (projectSetupError, projectModel) => {
                ...
                this.setState(
                  ...
                  () => {
                    ...
                    return this.awaitAllProjectModelsState(
                      projectPath,
                      "project:ready",
                      true,
                      () => {
                        ...
                        if (ac) {
                          ...
                          ac.setAsCurrentActiveComponent(
                            { from: "creator" },
                            () => {}
                          );
                        } else {
                          // And if we don't have anything assigned, assume we're editing the main component
                          this.state.projectModel.setCurrentActiveComponent(
                            "main",
                            { from: "creator" },
                            () => {}
                          );
  ...
  updateMenu() {
    ipcRenderer.send(
      "topmenu:update",
      this.state.projectModel.describeTopMenu()
    );
  }
  ...
  handleActiveComponentReady() {
    ...
    this.getActiveComponent().on("sustained-check:start", () => {
      ...
      const currentIdentifiersNotFound =
        activeComponent.sustainedWarningsChecker.notFoundIdentifiers;

      // If changed, delete current notice and display a new notice if num identifier not found > 0
      if (
        !lodash.isEqual(currentIdentifiersNotFound, this.identifiersNotFound)
      ) {
        // Delete old notice
        this.deleteIdentifierNotFoundNotice();
        ...
        if (currentIdentifiersNotFound.length > 0) {
          this.identifiersNotFoundNotice = this.createNotice({
            title: "Uh oh",
            type: "warning",
            message: `One or more of your expressions refers to an undefined state variable(s): ${currentIdentifiersNotFound.join(
              ", "
            )}. Please edit your expressions to remove these references.`
          });
        }
    ...
    if (this.state.projectFolder) {
      this.awaitAllProjectModelsState(
  ...
  mountHaikuComponent() {
    ...
    this.getActiveComponent().mountApplication(
      null,
      {
  ...
  shouldNoticeBeSkipped(notice) {
    ...
    if (
      notice.type === "error" &&
      notice.message.startsWith("Uncaught") &&
      shouldEmitErrors()
    ) {
      return true;
    }
  ...
  showForkingError() {
    ...
    this.createNotice({
      ...
      title: "Oh no!",
      message:
        "This project cannot be forked. 😢 This may have been disabled by the project's owner.",
      closeText: "Okay",
      lightScheme: true
    });
  }
  ...
  forkProject(organizationName, projectName) {
    mixpanel.haikuTrack("creator:fork-project", {
      organizationName,
      projectName
    });
    const doFork = () => {
    ...
    if (this.state.projectModel) {
      this.teardownMaster(
        { shouldFinishTour: true, doShowProjectLoader: true },
        doFork
      );
    } else {
      this.setState({ doShowProjectLoader: true });
  ...
  teardownMaster({ shouldFinishTour }, cb) {
    ...
    return this.props.websocket.request(
      {
        method: "teardownMaster",
        params: [this.state.projectModel.getFolder()]
      },
      () => {
        ...
        this.unsetAllProjectModelsState(
          this.state.projectModel.getFolder(),
          "project:ready"
        );
        this.unsetAllProjectModelsState(
          this.state.projectModel.getFolder(),
  ...
  logOut = () => {
    if (this.user) {
      this.user.logOut().then(() => {
        this.clearAuth();
        mixpanel.haikuTrack(
          "creator:project-browser:user-menu-option-selected",
          {
            option: "logout"
          }
        );
  ...
  showNewProjectModal(
    ...
    if (
      ...
      !this.envoyProject ||
      !this.user ||
      this.state.expiredTrialNonPro
    ) {
      return;
    }
    this.setState({
      showNewProjectModal: true,
      isDuplicateProjectModal,
      duplicateProjectName,
      projectToDuplicate
    });
  ...
  get proxyDescriptor() {
    ...
    return describeProxyFromUrl(
      this.props.haiku.dotenv.http_proxy || this.props.haiku.proxy.url
    );
  }
      ...
      (error, dotenv) => {
        if (error) {
          ...
          this.createNotice({
            ...
            title: "Oh no!",
            message:
              "We were unable to save your proxy settings. 😢 Please close and reopen the application and try again. If you still see this message, contact Haiku for support.",
            closeText: "Okay",
            lightScheme: true
          });
        } else {
          ...
          this.createNotice({
            ...
            message:
              "If you are still unable to connect to Haiku, please exit Haiku and try again.",
            closeText: "Okay",
            lightScheme: true
          });
  ...
  saveEventHandlers(targetElement, serializedEvents) {
    ...
    this.getActiveComponent().batchUpsertEventHandlers(
      selectorName,
      serializedEvents,
      { from: "creator" },
      () => {}
  ...
  handleShowConfirmGroupPopup(groupOrUngroup) {
    mixpanel.haikuTrack(
      `creator:show-confirm-group-ungroup-popup:${groupOrUngroup}`
    );
    this.setState({ showConfirmGroupUngroupPopup: true, groupOrUngroup });
    this.state.projectModel.broadcastPayload({
      name: "confirm-group-ungroup-popup-open",
      groupOrUngroup
    });
  }
  ...
  hideConfirmGroupUngroupPopup(userConfirmedGroup, groupOrUngroup) {
    ...
    this.state.projectModel.broadcastPayload({
      name: "confirm-group-ungroup-popup-closed",
      confirmed: userConfirmedGroup,
      groupOrUngroup
    });
  }
  ...
  conglomerateComponent = (
    { isBlankComponent, skipInstantiateInHost } = {
  ...
  get showAuthenticationUI() {
    return (
      this.state.readyForAuth &&
      (!this.state.isUserAuthenticated || !this.state.username)
    );
  }
  ...
  render() {
    ...
    return (
      <div style={{ position: "relative", width: "100%", height: "100%" }}>
        <CSSTransition classNames="toast" timeout={{ enter: 500, exit: 300 }}>
        ...
        {this.shouldShowProjectLoader() && (
          <ProjectLoader
            softwareVersion={this.state.softwareVersion}
            message={
              this.showGenericLoader ? "Loading…" : "Initializing project…"
            }
          />
        )}
        {!this.state.dashboardVisible &&
                    ...
                    >
                      ...
                      {this.state.showConfirmGroupUngroupPopup && (
                        <ConfirmGroupUngroupPopup
                          user={this.user}
                          setGroupUngroupAnswerAndClose={
                            this.hideConfirmGroupUngroupPopup
                          }
                      ...
                      <Stage
                        ...
                        setGlassInteractionToPreviewMode={
                          this.setGlassInteractionToPreviewMode
                        }
                        setGlassInteractionToEditMode={
                          this.setGlassInteractionToEditMode
                        }
                        setGlassInteractionToCodeEditorMode={
                          this.setGlassInteractionToCodeEditorMode
                        }
                        tryToChangeCurrentActiveComponent={
                          this.tryToChangeCurrentActiveComponent
                        }
                        showEventHandlerEditor={
                          this.state.showEventHandlerEditor
                        }
                        conglomerateComponent={this.conglomerateComponent}
                      />
        ...
        {this.state.showFailWhale && (
          <FailWhale
            restart={this.restart}
            uniqueId={this.state.failWhaleUniqueId}
          />
        )}
      </div>
    );

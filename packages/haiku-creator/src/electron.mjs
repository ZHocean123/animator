...
if (mode === 0) {
  //You choose to create a new project.  You take a deep breath, then:
  ...
  while (!folderIsEmpty) {
    dialog.showMessageBox({
      message:
        "On the coming screen, select an empty directory for this project.",
      type: "info",
      buttons: ["OK"]
    });
  ...
  //You are sure that Open is the way to go.  Without hesitation, you charge ahead:
  dialog.showMessageBox({
    message:
      "On the coming screen, select a directory containing a Haiku Animator project. \r\n\r\nFor legacy commercial projects, check ~/.haiku/projects",
    type: "info",
    buttons: ["OK"]
  });
  ...
  //You turn around and head back from whence you came.
  process.kill(0);
}
...
const handleUrl = url => {
  if (!browserWindow) {
    logger.warn(
      `[creator] unable to handle custom protocol URL ${url}; browserWindow not ready`
    );
    return;
  }
...
if (!haiku.plumbing.url) {
  if (
    ...
    !global.process.env.HAIKU_PLUMBING_PORT
  ) {
    throw new Error(`Oops! You must define a HAIKU_PLUMBING_PORT env var!`);
  }

  // tslint:disable-next-line:max-line-length
  haiku.plumbing.url = `http://${global.process.env.HAIKU_PLUMBING_HOST ||
    "0.0.0.0"}:${global.process.env.HAIKU_PLUMBING_PORT}/?token=${
  ...
  if (!isMac()) {
    const isSecondInstance = app.makeSingleInstance(
      (commandLine, workingDirectory) => {
        logger.info(
          `[creator] Received command line on second instance ${commandLine}`
        );

 ...
 * @private
 */
function animatedTimelineReducer(accumulator: any, currentValue: any) {
 ...
 * @returns {{}}
 */
export function compoundTimelineReducer(accumulator: any, currentValue: any) {
  ...
  return {
    ...accumulator,
    [PropertyKey.Value]: accumulator[PropertyKey.Value].concat(currentValue[PropertyKey.Value]),
  }
}
 ...
 */
export function getFixedPropertyValue(fixedValue: any): BodymovinProperty {
  return {
 ...
 * @returns {any[]}
 */
export function alwaysArray(maybeArray: any): any[] {
 ...
 * @returns {number}
 */
export function alwaysAbsolute(maybePercent: string | number, basis: number): number {
  if (typeof maybePercent === 'string' && maybePercent.endsWith('%')) {
    return Number.parseFloat(maybePercent.replace('%', '')) * basis / 100
  }
 ...
 */
export function getShapeDimensions(shape: any): BodymovinDimensions {
 ...
 * @returns {any}
 */
export function maybeApplyMutatorToProperty(property: any, mutator: (param: any) => any, disableRecursion: boolean = false) {
 ...
 * @private
 */
function translateInterpolationPoints(points: BodymovinPathComponent, vertices: BodymovinPathComponent) {
 ...
 * @param points
 */
export function pathToInterpolationTrace(points: CurveSpec[]) {
 ...
 * @returns {[key in PathKey]: BodymovinPathComponent}
 */
export function pointsToInterpolationTrace(svgPoints: string | [number, number][]) {
  ...
  const dummyCurve = Array.from({ length: chunkedPoints.length })
  dummyCurve.fill([0, 0])
  return {
    [PathKey.Closed]: true,
 ...
 * @returns {string[]}
 */
export function decomposePath(path: string | CurveSpec[]): { points: CurveSpec[], closed: boolean }[] {
 ...
 * @returns {number[]}
 */
export function keyframesFromTimelineProperty(timelineProperty: BytecodeTimelineProperty): number[] {
  const keyframes: number[] = Object.keys(timelineProperty).map(i => Number.parseInt(i, 10))
 ...
 */
export function timelineValuesAreEquivalent(valueA: any, valueB: any): boolean {
 ...
 * @returns {object}
 */
export function lottieAndroidStreamSafeToJson() {

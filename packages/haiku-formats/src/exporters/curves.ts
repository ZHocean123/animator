 ...
 */
export function getCurveInterpolationPoints(curve: Curve | InterpolationPoints) {
 ...
 * @private
 */
function reinterpolateBezier(points: number[][], dimension: number, time: number) {
 ...
 * @private
 */
function denormalizeValue(normalizedValue: number, from: number, to: number): number {
  return from + normalizedValue * (to - from)
}
 ...
 * @private
 */
function normalizeValue(value: number, from: number, to: number): number {
 ...
 */
export function splitBezierForTimelinePropertyAtKeyframe(timelineProperty: BytecodeTimelineProperty, keyframe: number) {
 ...
 * @private
 */
function composeBezierBreakpoints(first: BezierBreakpoint[], second: BezierBreakpoint[]): BezierBreakpoint[] {
  return first.map(([time, value, curve]) => [time / 2, value / 2, curve])
 ...
 * @private
 */
function getBezierBreakpointsForDecomposableCurve(curve: Curve): BezierBreakpoint[] {
  switch (curve) {
 ...
 * @private
 */
const isElasticCurve = (curve: Curve) => curve.endsWith('Elastic')
 ...
 * @param outKeyframe
 */
export function decomposeCurveBetweenKeyframes(timelineProperty: BytecodeTimelineProperty, inKeyframe: number, outKeyframe: number) {

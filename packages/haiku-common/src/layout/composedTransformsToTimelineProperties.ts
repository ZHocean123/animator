...
function cleanInvalidOrOverexplicitProps(out: ComposedTransformSpec, explicit = false) {
 ...
 */
function normalizeRotationsInQuadrants(out: ComposedTransformSpec, normalizer: LayoutSpec, epsilon: number = 1e3) {
  if (normalizer === null) {
    return
  }

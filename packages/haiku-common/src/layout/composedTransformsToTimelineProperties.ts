...
const cleanInvalidOrOverexplicitProps = (
  ...
  Object.keys(out).forEach(layoutPropertyName => {
    switch (layoutPropertyName) {
      ...
      case "scale.z":
        if (
          isNaN(out[layoutPropertyName]) ||
          (!explicit && out[layoutPropertyName] === 1)
        ) {
      ...
      case "shear.yz":
        if (
          isNaN(out[layoutPropertyName]) ||
          (!explicit && out[layoutPropertyName] === 0)
        ) {
          delete out[layoutPropertyName];
        }
        break;
 ...
 */
const normalizeRotationsInQuadrants = (
  out: ComposedTransformSpec,
  normalizer: LayoutSpec,
  epsilon: number = 1e3
) => {
  if (normalizer === null) {
    return;
  ...
  ["x", "y", "z"].forEach(axis => {
    ...
    const originalQuadrant = Math.floor(
      (2 * (normalizer.rotation as any)[axis]) / Math.PI
    );
    ...
    out[rotationProperty] +=
      Math.PI * 2 * Math.round((originalQuadrant - quadrantOut) / 4);
    // …and round to avoid additional weirdness.
    out[rotationProperty] =
      Math.round(out[rotationProperty] * epsilon) / epsilon;
  });
};
...
export default (
  ...
  const decomposed = mat4Decompose(
    Layout3D.multiplyArrayOfMatrices(matrices.reverse()),
    epsilon
  );

...
import * as cssValueModule from "haiku-vendor-legacy/lib/css-value/index.js";

const cssValue = cssValueModule.default;
...
export default (mana: BytecodeNode): BytecodeNode => {
  visitManaTree(
    ROOT_LOCATOR,
    mana,
    (name, attributes, children, node, locator, parent, index) => {
      ...
      if (typeof name !== "string" || !(SVG_SIZEABLES as any)[name]) {
        ...
        if (attributes.style && typeof attributes.style === "object") {
          ...
          if (
            ...
            const heightStyleProp = determineSizingProp(
        ...
        if (!parent) {
          ...
          if (
            !isNumericDefined(attributes["sizeAbsolute.x"]) &&
            attributes["sizeAbsolute.x"] !== AUTO_SIZING_TOKEN
          ) {
            attributes["sizeAbsolute.x"] = fallbackSizeAbsolute(node, "x");
          }
          if (
            !isNumericDefined(attributes["sizeAbsolute.y"]) &&
            attributes["sizeAbsolute.y"] !== AUTO_SIZING_TOKEN
          ) {
      ...
      if (attributes.transform !== undefined && attributes.transform !== null) {
        const transformAttributes = parseCssTransformString(
          attributes.transform,
          1e6
        );

        for (const transformAttributeName in transformAttributes) {
          const transformValue = (transformAttributes as any)[

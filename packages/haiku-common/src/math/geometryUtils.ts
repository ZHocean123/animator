...
export function bezierCubic(a: Vec2, h1: Vec2, h2: Vec2, b: Vec2, t: number): Vec2 {
  const t2 = t * t
  const t3 = t2 * t
  const mt = 1 - t
  const mt2 = mt * mt
...
export function buildPathLUT(points: CurveSpec[], segmentResolution: number = CUBIC_BEZIER_APPROXIMATION_RESOLUTION): [Vec2[], boolean] {
  const out = []
...
export function pointInsideRect(pt: Vec2, corner1: Vec2, corner2: Vec2): boolean {
  const c1 = { x: 0, y: 0 }
  const c2 = { x: 0, y: 0 }
...
function evenOddRaycastPointInPolygon(points: Vec2[], test: Vec2): boolean {
  let intersections = 0
  let j
...
function vec2MulScalar(a: Vec2, n: number): Vec2 {
  return { x: a.x * n, y: a.y * n }
}

function vec2DivScalar(a: Vec2, n: number): Vec2 {
  return { x: a.x / n, y: a.y / n }
}
...
function vec2Mag(a: Vec2): number {
  return Math.sqrt(a.x * a.x + a.y * a.y)
}

function vec2Normalize(a: Vec2): Vec2 {
  const mag = vec2Mag(a)
  if (mag) {
...
export function closestNormalPointOnLineSegment(a: Vec2, b: Vec2, test: Vec2): Vec2 {
...
export function pointOnLineSegment(a: Vec2, b: Vec2, test: Vec2, threshold: number = DEFAULT_LINE_SELECTION_THRESHOLD): boolean {
  return distance(closestNormalPointOnLineSegment(a, b, test), test) <= threshold
}

export function pointOnPolyLineSegment(points: Vec2[], test: Vec2, threshold: number = DEFAULT_LINE_SELECTION_THRESHOLD): boolean {
  for (let i = 1; i < points.length; i++) {
...
export function transform2DPoint(point: Vec2, ancestryMatrices: any[]): Vec2 {
...
export function isPointAlongStroke(element: HaikuElement, point: Vec2, threshold: number = DEFAULT_LINE_SELECTION_THRESHOLD): boolean {
  ...
  switch (element && element.type) {
    ...
    case 'ellipse': {
      const a
        = (correctedPoint.x - Number(element.attributes.cx)) ** 2 / Number(element.attributes.rx) ** 2
          + (correctedPoint.y - Number(element.attributes.cy)) ** 2 / Number(element.attributes.ry) ** 2
      return a > 0.9 && a < 1.1 // TODO: Some actually good math here! Need to take stroke width into account
    }
    case 'line':
...
export function isPointInsidePrimitive(element: HaikuElement, point: Vec2): boolean {
  const original = element
  if (element.type === 'use') {
  ...
  switch (element.type) {
    ...
    case 'ellipse':
      return (correctedPoint.x - Number(element.attributes.cx)) ** 2 / Number(element.attributes.rx) ** 2 + (correctedPoint.y - Number(element.attributes.cy)) ** 2 / Number(element.attributes.ry) ** 2 <= 1

...
import HaikuElement from "@haiku/core/lib/HaikuElement.js";
import { distance, Vec2 } from "@haiku/core/lib/helpers/PathUtils.js";
...
export const isClosedPath = (maybePath: CurveSpec | null): boolean =>
  !!(maybePath && maybePath.closed);
...
export const buildPathLUT = (
  ...
  return [
    out,
    isClosedPath(points[points.length - 1]) ||
      isClosedPath(points[points.length - 2])
  ];
};
...
export const closestNormalPointOnLineSegment = (
  a: Vec2,
  b: Vec2,
  test: Vec2
): Vec2 => {
...
export const pointOnLineSegment = (
  ...
  b: Vec2,
  test: Vec2,
  threshold: number = DEFAULT_LINE_SELECTION_THRESHOLD
): boolean => {
  return (
    distance(closestNormalPointOnLineSegment(a, b, test), test) <= threshold
  );
};
...
export const transform2DPoint = (
  point: Vec2,
  ancestryMatrices: any[]
): Vec2 => {
  ...
  return {
    x:
      invertedOffset[0] * p[0] +
      invertedOffset[4] * p[1] +
      invertedOffset[8] * p[2] +
      invertedOffset[12] * p[3],
    y:
      invertedOffset[1] * p[0] +
      invertedOffset[5] * p[1] +
      invertedOffset[9] * p[2] +
      invertedOffset[13] * p[3]
  };
};

export const isPointAlongStroke = (
  ...
  const correctedPoint = transform2DPoint(
    point,
    original.layoutAncestryMatrices.reverse()
  );
  ...
  switch (element && element.type) {
    case "rect": {
      ...
      const p2 = {
        x: Number(element.attributes.x) + element.sizeX,
        y: Number(element.attributes.y)
      };
      const p3 = {
        x: Number(element.attributes.x) + element.sizeX,
        y: Number(element.attributes.y) + element.sizeY
      };
      const p4 = {
        x: Number(element.attributes.x),
        y: Number(element.attributes.y) + element.sizeY
    ...
    case "circle": {
      const dist = distance(correctedPoint, {
    ...
    case "ellipse": {
      const a =
        Math.pow(correctedPoint.x - Number(element.attributes.cx), 2) /
          Math.pow(Number(element.attributes.rx), 2) +
        Math.pow(correctedPoint.y - Number(element.attributes.cy), 2) /
          Math.pow(Number(element.attributes.ry), 2);
    ...
    case "polygon":
    case "polyline": {
      const polyPoints = SVGPoints.polyPointsStringToPoints(
        element.attributes.points
    ...
    case "path": {
      ...
      const [points, closed] = buildPathLUT(
        SVGPoints.pathToPoints(element.attributes.d)
      );
      if (closed) {
...
export const isPointInsidePrimitive = (
  element: HaikuElement,
  point: Vec2
): boolean => {
  const original = element;

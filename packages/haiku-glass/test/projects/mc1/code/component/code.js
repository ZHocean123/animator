var Haiku = require("packages/@haiku/core/src/index.ts");
module.exports = {
  metadata: {
    uuid: "HAIKU_SHARE_UUID",
    type: "haiku",
    name: "UsersMatthewCodeHaikuTeamMonoPackagesHaikuPlumbingTestFixturesProjectsBlankProject",
    relpath: "code/component/code.js",
    core: "3.2.12"
  },
  options: {},
  states: {},
  eventHandlers: {},
  timelines: {
    Default: {
      "haiku:f8dfa2025368": {
        "style.WebkitTapHighlightColor": { "0": { value: "rgba(0,0,0,0)" } },
        "style.transformStyle": { "0": { value: "flat" } },
        "style.perspective": { "0": { value: "none" } },
        "style.position": { "0": { value: "relative" } },
        "style.overflowX": { "0": { value: "hidden" } },
        "style.overflowY": { "0": { value: "hidden" } },
        "sizeAbsolute.x": { "0": { value: 550 } },
        "sizeAbsolute.y": { "0": { value: 400 } },
        "sizeMode.x": { "0": { value: 1 } },
        "sizeMode.y": { "0": { value: 1 } },
        "sizeMode.z": { "0": { value: 1 } },
        "translation.x": { "0": { value: 0 } },
        "translation.y": { "0": { value: 0 } }
      },
      "haiku:18cd0a9f4a8e": {
        "style.position": { "0": { value: "absolute" } },
        "style.margin": { "0": { value: "0" } },
        "style.padding": { "0": { value: "0" } },
        "style.border": { "0": { value: "0" } },
        "sizeAbsolute.x": { "0": { value: 115 } },
        "sizeMode.x": { "0": { value: 1 } },
        "sizeAbsolute.y": { "0": { value: 115 } },
        "sizeMode.y": { "0": { value: 1 } },
        "translation.x": { "0": { value: 275 } },
        "translation.y": { "0": { value: 200 } },
        "style.zIndex": { "0": { value: 1 } }
      },
      "haiku:f87e1ceaed3c": {
        stroke: { "0": { value: "none" } },
        "stroke-width": { "0": { value: "1" } },
        fill: { "0": { value: "none" } },
        "fill-rule": { "0": { value: "evenodd" } }
      },
      "haiku:e0dbd6ae559c": {
        "translation.x": { "0": { value: -189 } },
        "translation.y": { "0": { value: -622 } }
      },
      "haiku:cb5838e4af1d": {
        fill: { "0": { value: "#5DE2F9" } },
        cx: { "0": { value: "246.5" } },
        cy: { "0": { value: "679.5" } },
        r: { "0": { value: "57.5" } }
      },
      "haiku:90da86e1f8b9": {
        "style.position": { "0": { value: "absolute" } },
        "style.margin": { "0": { value: "0" } },
        "style.padding": { "0": { value: "0" } },
        "style.border": { "0": { value: "0" } },
        "sizeAbsolute.x": { "0": { value: 115 } },
        "sizeMode.x": { "0": { value: 1 } },
        "sizeAbsolute.y": { "0": { value: 115 } },
        "sizeMode.y": { "0": { value: 1 } },
        "translation.x": { "0": { value: 275 } },
        "translation.y": { "0": { value: 200 } },
        "style.zIndex": { "0": { value: 2 } }
      },
      "haiku:f680c9408abc": {
        stroke: { "0": { value: "none" } },
        "stroke-width": { "0": { value: "1" } },
        fill: { "0": { value: "none" } },
        "fill-rule": { "0": { value: "evenodd" } }
      },
      "haiku:1594ce9f4cd1": {
        "translation.x": { "0": { value: -189 } },
        "translation.y": { "0": { value: -622 } }
      },
      "haiku:c05f2d361fc2": {
        fill: { "0": { value: "#5DE2F9" } },
        cx: { "0": { value: "246.5" } },
        cy: { "0": { value: "679.5" } },
        r: { "0": { value: "57.5" } }
      }
    }
  },
  template: {
    elementName: "div",
    attributes: {
      "haiku-id": "f8dfa2025368",
      "haiku-title": "UsersMatthewCodeHaikuTeamMonoPackagesHaikuPlumbingTestFixturesProjectsBlankProject"
    },
    children: [
      {
        elementName: "svg",
        attributes: {
          version: "1.1",
          xmlns: "http://www.w3.org/2000/svg",
          "xmlns:xlink": "http://www.w3.org/1999/xlink",
          source: "designs/UsersMatthewCodeHaik.sketch.contents/slices/blue-circle.svg",
          "haiku-id": "90da86e1f8b9",
          "haiku-title": "blue-circle"
        },
        children: [
          {
            elementName: "g",
            attributes: { "haiku-id": "f680c9408abc", id: "e86a86bc8ac7" },
            children: [
              {
                elementName: "g",
                attributes: { "haiku-id": "1594ce9f4cd1", id: "66ea068ce385" },
                children: [
                  {
                    elementName: "circle",
                    attributes: {
                      "haiku-id": "c05f2d361fc2",
                      id: "243f67d33a33"
                    },
                    children: []
                  }
                ]
              }
            ]
          }
        ]
      },
      {
        elementName: "svg",
        attributes: {
          version: "1.1",
          xmlns: "http://www.w3.org/2000/svg",
          "xmlns:xlink": "http://www.w3.org/1999/xlink",
          source: "designs/UsersMatthewCodeHaik.sketch.contents/slices/blue-circle.svg",
          "haiku-id": "18cd0a9f4a8e",
          "haiku-title": "blue-circle"
        },
        children: [
          {
            elementName: "g",
            attributes: { "haiku-id": "f87e1ceaed3c", id: "ad9765bcb241" },
            children: [
              {
                elementName: "g",
                attributes: { "haiku-id": "e0dbd6ae559c", id: "5175c4baab53" },
                children: [
                  {
                    elementName: "circle",
                    attributes: {
                      "haiku-id": "cb5838e4af1d",
                      id: "e84bcd5506e7"
                    },
                    children: []
                  }
                ]
              }
            ]
          }
        ]
      }
    ]
  }
};

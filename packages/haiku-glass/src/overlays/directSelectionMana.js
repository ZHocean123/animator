...
function anchorPoint(index, meta, selected, scale, { x, y }) {
  return {
    elementName: 'g',
    attributes: {
...
function getLogicalAnchorsFromPoints(points) {
...
export function rect(id, { x, y, width, height, rx, ry }, layoutAncestry, controlPointScale, selectedAnchorIndices) {
...
export function circle(id, { cx, cy, r }, layoutAncestry, controlPointScale, selectedAnchorIndices) {
  const transform = Layout3D.multiplyArrayOfMatrices(layoutAncestry.reverse())
  const transposed = Layout3D.createMatrix()
  transpose(transposed, transform)
...
export function ellipse(id, { cx, cy, rx, ry }, layoutAncestry, controlPointScale, selectedAnchorIndices) {
  const transform = Layout3D.multiplyArrayOfMatrices(layoutAncestry.reverse())
  const transposed = Layout3D.createMatrix()
  transpose(transposed, transform)
...
export function polygon(id, { points }, layoutAncestry, controlPointScale, selectedAnchorIndices) {
  const transform = Layout3D.multiplyArrayOfMatrices(layoutAncestry.reverse())
...
export function path(id, { d }, layoutAncestry, controlPointScale, selectedAnchorIndices) {
  const transform = Layout3D.multiplyArrayOfMatrices(layoutAncestry.reverse())
  transpose(transform, transform)
...
export function line(id, { x1, y1, x2, y2 }, layoutAncestry, controlPointScale, selectedAnchorIndices) {
  const transform = Layout3D.multiplyArrayOfMatrices(layoutAncestry.reverse())
  transpose(transform, transform)
...
export function polyline(id, { points }, layoutAncestry, controlPointScale, selectedAnchorIndices) {
  const transform = Layout3D.multiplyArrayOfMatrices(layoutAncestry.reverse())
  transpose(transform, transform)
  const pts = SVGPoints.polyPointsStringToPoints(points)
  for (const i in pts) {

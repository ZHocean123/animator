...
import Config from '@haiku/core/Config'
import HaikuContext from '@haiku/core/HaikuContext'
import { distance, splitSegmentInSVGPoints } from '@haiku/core/helpers/PathUtils'
import SVGPoints from '@haiku/core/helpers/SVGPoints'
import HaikuDOMRenderer from '@haiku/core/renderers/dom'
import { calculateValue } from '@haiku/core/Transitions'
...
import { EnvoyClient, ERROR_CHANNEL } from 'haiku-sdk-creator'

import { Asset, BaseModel, Element, ElementSelectionProxy, EmitterManager, File, HOMEDIR_PATH, isCoordInsideBoxPoints, logger, mixpanel, Project, requestElementCoordinates, Template } from 'haiku-serialization'
import { ComponentIconSVG, Globals, Palette, PopoverMenu } from 'haiku-ui-common'
...
export class Glass extends React.Component {
  ...
  handleMouseDown(mousedownEvent) {
    ...
    switch (mousedownEvent.nativeEvent.target.getAttribute('class')) {
      case 'direct-selection-anchor': {
        const dataIndex = Number.parseInt(mousedownEvent.nativeEvent.target.getAttribute('data-index'), 10)
        // NOTE: meta used to determine if anchor or handle for <path> (see directSelectionMana.js)
        const meta = mousedownEvent.nativeEvent.target.getAttribute('data-meta') && mousedownEvent.nativeEvent.target.getAttribute('data-meta').length ? Number.parseInt(mousedownEvent.nativeEvent.target.getAttribute('data-meta'), 10) : null
  ...
  deselectAllOtherElementsIfTargetNotAmongThem(target, cb) {
    ...
    const isAmongSelection = selecteds.includes(target)
    if (!isAmongSelection) {
  ...
  getCSSTransform(zoom, pan) {
    return `matrix3d(${
      [zoom.x, 0, 0, 0, 0, zoom.y, 0, 0, 0, 0, 1, 0, pan.x / zoom.x, pan.y / zoom.x, 0, 1].join(',')})`
  }
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <div
          ref={(ref) => { this.refs.container = ref }}

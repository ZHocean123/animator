...
export function isCoordInsideRect(px, py, rect) {
  return rect.left <= px && px <= rect.right && rect.top <= py && py <= rect.bottom
}

export function rounded(n, d = 3) {
  const powerOfTen = 10 ** d
  return Math.round(n * powerOfTen) / powerOfTen
}

export const basicallyEquals = (n, m) => typeof n === 'number' && typeof m === 'number' && Math.abs(n - m) < 1e-3

export function isCoordInsideBoxPoints(px, py, boxPoints) {
...
export function modOfIndex(idx, max) {
...
export function transformFourVectorByMatrix(out, v, m) {

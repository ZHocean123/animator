...
import { default as convertManaLayout } from 'haiku-common/lib/layout/convertManaLayout.mjs';
...
const FALLBACK_TEMPLATE =
  '<' + DEFAULT_ROOT_NODE_NAME + '></' + DEFAULT_ROOT_NODE_NAME + '>';
...
Bytecode.areBytecodesIsomorphic = (b1, b2) => {
  ...
  const areTemplates = Template.areTemplatesEquivalent(
    b1.template,
    b2.template,
  );
...
Bytecode.getAppliedStatesForNode = (out, bytecode, node) => {
      ...
      for(const propertyName in bytecode.timelines[timelineName][selector]) {
        for(const keyframeMs in bytecode.timelines[timelineName][selector][
          ...
          if(typeof keyframeObj.value === 'function') {
            ...
            allExprParams.push.apply(
              allExprParams,
              keyframeObj.value.specification.params,
            );
          }
        }
      }
  ...
  allExprParams.forEach(paramName => {
    if(typeof paramName === 'string') {
      // In case of legacy destructured objects
    ...
    for(const timelineName in bytecode.timelines) {
      for(const timelineSelector in bytecode.timelines[timelineName]) {
        ...
        if(
          ...
          out[timelineName][timelineSelector] =
            bytecode.timelines[timelineName][timelineSelector];
        }
      }
    }
  });
      ...
      for(const timelineName in bytecode.timelines) {
        ...
        for(const timelineSelector in timelineObject) {
          for(const propertyName in timelineObject[timelineSelector]) {
            ...
            for(const keyframeMs in timelineObject[timelineSelector][
              propertyName
            ]) {
              const propertyValue =
                timelineObject[timelineSelector][propertyName][keyframeMs]
                  .value;
              const fixedValue = fixedReferences[propertyValue];
              if(fixedValue) {
                timelineObject[timelineSelector][propertyName][
                  keyframeMs
                ].value = fixedValue;
              }
            }
    ...
    for(const eventSelector in b2.eventHandlers) {
      for(const eventName in b2.eventHandlers[eventSelector]) {
        ...
        if(
          ...
          (previousEventHandler && !previousEventHandler.edited)
        ) {
          if(!b1.eventHandlers[eventSelector]) {
            b1.eventHandlers[eventSelector] = {};
          }
          b1.eventHandlers[eventSelector][eventName] =
            b2.eventHandlers[eventSelector][eventName];
        }
      }
    }
  }
  ...
  for(const timelineName in t2) {
    for(const timelineSelector in t2[timelineName]) {
      for(const propertyName in t2[timelineName][timelineSelector]) {
        for(const keyframeMs in t2[timelineName][timelineSelector][
          propertyName
        ]) {
          ...
          if(
            !previousKeyframe ||
            (previousKeyframe && !previousKeyframe.edited)
          ) {
            if(!t1[timelineName]) {
              t1[timelineName] = {};
            }
            ...
            const targetObj =
              t1[timelineName][timelineSelector][propertyName][keyframeMs];
            const sourceObj =
              t2[timelineName][timelineSelector][propertyName][keyframeMs];
  ...
  if(pasted.eventHandlers) {
    ...
    for(const eventSelector in pasted.eventHandlers) {
      for(const eventName in pasted.eventHandlers[eventSelector]) {
        ...
        destination.eventHandlers[eventSelector][eventName] =
          pasted.eventHandlers[eventSelector][eventName];
      }
    }
  }
  ...
  if(pasted.timelines) {
    ...
    for(const timelineName in pasted.timelines) {
      for(const timelineSelector in pasted.timelines[timelineName]) {
        for(const propertyName in pasted.timelines[timelineName][
          timelineSelector
        ]) {
          for(const keyframeMs in pasted.timelines[timelineName][
            timelineSelector
          ][propertyName]) {
            ...
            destination.timelines[timelineName][timelineSelector][propertyName][
              ...
              pasted.timelines[timelineName][timelineSelector][propertyName][
                keyframeMs
              ];
          }
        }
...
Bytecode.decycle = (reified, { cleanManaOptions = {}, doCleanMana }) => {
  ...
  if(reified.eventHandlers) {
    ...
    for(const componentId in reified.eventHandlers) {
      ...
      for(const eventListenerName in reified.eventHandlers[componentId]) {
        const handlerToAssign =
          reified.eventHandlers[componentId][eventListenerName].handler;
  ...
  if(reified.template) {
    if(doCleanMana) {
      decycled.template = Template.cleanMana(
        reified.template,
        cleanManaOptions,
      );
    } else {
...
Bytecode.reinitialize = (folder, relpath, bytecode = {}, config = {}) => {
  ...
  for(const timelineName in bytecode.timelines) {
    for(const selector in bytecode.timelines[timelineName]) {
      ...
      for(const property in bytecode.timelines[timelineName][selector]) {
        if(
          typeof bytecode.timelines[timelineName][selector][property] !==
          'object'
        ) {
  ...
  Bytecode.upsertDefaultProperties(
    bytecode,
    contextHaikuId,
    {
...
Bytecode.upsertDefaultProperties = (
  ...
  for(const propName in propertiesToMerge) {
    ...
    switch(strategy) {
      case 'merge':
        defaultTimeline[propName][DEFAULT_TIMELINE_TIME].value =
          propertiesToMerge[propName];
        break;
      case 'assign':
        if(
          defaultTimeline[propName][DEFAULT_TIMELINE_TIME].value === undefined
        ) {
          defaultTimeline[propName][DEFAULT_TIMELINE_TIME].value =
            propertiesToMerge[propName];
        }
...
Bytecode.batchUpsertEventHandlers = (
  ...
  Object.entries(serializedEvents).forEach(([event, handlerDescriptor]) => {
    ...
    if(handlerDescriptor.handler !== undefined) {
      bytecode.eventHandlers[selectorName][
        event
      ].handler = Bytecode.unserializeValue(handlerDescriptor.handler);
    }
  });
...
Bytecode.changeKeyframeValue = (
  ...
  const property = Bytecode.ensureTimelineProperty(
    bytecode,
    timelineName,
    componentId,
    propertyName,
...
Bytecode.createKeyframe = (
  ...
  const precedingValue = TimelineProperty.getPropertyValueAtTime(
    bytecode.timelines,
    timelineName,
    componentId,
    elementName,
  ...
  const currentValue = TimelineProperty.getPropertyValueAtTime(
    bytecode.timelines,
    timelineName,
    componentId,
    elementName,
  ...
  const precedingAssignedValueObject = TimelineProperty.getAssignedBaselineValueObject(
    componentId,
    elementName,
    propertyName,
    timelineName,
  ...
  const precedingAssignedValue =
    precedingAssignedValueObject && precedingAssignedValueObject.value;
  const currentAssignedValueObject = TimelineProperty.getAssignedBaselineValueObject(
    componentId,
    elementName,
    propertyName,
    timelineName,
  ...
  const currentAssignedValue =
    currentAssignedValueObject && currentAssignedValueObject.value;
  ...
  if(keyframeEndMs !== undefined && keyframeEndMs !== null) {
    ...
    property[keyframeEndMs].value = Bytecode.unserializeValue(
      keyframeEndValue || keyframeValue,
    );
...
Bytecode.ensureTimelineProperty = (
  bytecode,
  timelineName,
  componentId,
  propertyName,
) => {
  const group = Bytecode.ensureTimelineGroup(
    bytecode,
    timelineName,
    componentId,
  );
  ...
  for(const timelineName in keyframeMoves) {
    for(const componentId in keyframeMoves[timelineName]) {
      for(const propertyName in keyframeMoves[timelineName][componentId]) {
        ...
        const keyframeMove = Bytecode.unserializeValue(
          keyframeMoves[timelineName][componentId][propertyName],
        );
...
// code file
Bytecode.unserializeValue = (
  value,
  referenceEvaluator = referenceEvaluatorMissing,
) => {
...
Bytecode.upsertStateValue = (bytecode, stateName, stateDescriptor) => {
  ...
  if(stateDescriptor.get !== undefined) {
    bytecode.states[stateName].get = Bytecode.unserializeValue(
      stateDescriptor.get,
    );
  }

  if(stateDescriptor.set !== undefined) {
    bytecode.states[stateName].set = Bytecode.unserializeValue(
      stateDescriptor.set,
    );
  }
...
Bytecode.upsertPropertyValue = (
  ...
  for(const propName in propertiesToMerge) {
    ...
    switch(strategy) {
      case 'merge':
        defaultTimeline[propName][timelineTime].value =
          propertiesToMerge[propName];
        break;
      case 'assign':

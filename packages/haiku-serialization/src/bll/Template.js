...
const { default: convertManaLayout } = require('haiku-common')
...
Template.mergeAppliedValue = (name, valueDescriptor, incomingValue, mergeStrategy) => {
  if (isObject(valueDescriptor.value) && isObject(incomingValue) && !isSerializedFunction(valueDescriptor.value) && !isSerializedFunction(incomingValue)) {
    ...
    switch (mergeStrategy) {
      ...
      case MERGE_STRATEGIES.defaults: if (valueDescriptor.value === undefined) {
...
Template.getStackingInfo = (
  ...
  return staticTemplateManaNode.children
    ...
    .map((child, index) => {
      ...
      const zIndex = Number.parseInt(Template.getPropertyValue(
        bytecode,
        haikuId,
        timelineName,
        timelineTime,
...
Template.getPropertyValue = (
  ...
  if (!bytecode.timelines[timelineName][`haiku:${componentId}`][propertyName][timelineTime]) {
    return
  }

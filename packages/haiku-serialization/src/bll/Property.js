...
const ROOT_CHILD_ONLY = (name, element, property, keyframes) =>
  element.isVisuallySelectable;
...
const IF_IN_SCHEMA = (name, element) =>
  hasInSchema(element.getSafeDomFriendlyName(), name);
...
Property.includeInDisplay = (type, name, element, property, keyframes) => {
  ...
  return (
    rule &&
    rule[type] &&
    rule[type].every(test => test(name, element, property, keyframes))
  );
};

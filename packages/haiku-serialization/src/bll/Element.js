...
class Element extends BaseModel {
  ...
  toggleLocked(metadata, cb) {
    this.component.setLockedStatusForComponent(
      this.getComponentId(),
      ...
      },
      {
        ...
        layout: {
          computed: {
            ...
            bounds:
              (this.parent &&
                this.parent.getHaikuElement().computeContentBounds()) ||
              {},
            size:
  ...
  setTitle(newTitle, metadata, cb) {
    this.component.setTitleForComponent(
      this.getComponentId(),
      newTitle,
      metadata,
      cb,
  ...
  getHostedPropertyRows(doRecurse = false) {
    ...
    if(doRecurse && experimentIsEnabled(Experiment.ShowSubElementsInJitMenu)) {
      ...
      this.visitDescendants(descendantElement => {
        ...
        const currentHeadingRow =
          descendantElement.topmostHeadingRow || headingRow;
  ...
  getLiveRenderedNode() {
    ...
    return instance
      ? instance.findElementsByHaikuId(this.getComponentId())[0]
      : null;
  }
  ...
  ungroupSvg(nodes) {
    ...
    ungroupables.forEach(descendantHaikuElement => {
      ...
      layoutMatrix[12] +=
        (boundingBox.x + originX) * layoutMatrix[0] +
        (boundingBox.y + originY) * layoutMatrix[4];
      layoutMatrix[13] +=
        (boundingBox.x + originX) * layoutMatrix[1] +
...
Element.buildPrimaryKeyFromComponentParentIdAndStaticTemplateNode = (
  component,
  parentId,
  indexInParent,
  staticTemplateNode,

const TimelineProperty = require('haiku-serializationbll/TimelineProperty')
...
class Row extends BaseModel {
  ...
  rehydrateKeyframes() {
    ...
    const keyframesList = Object.keys(valueGroup)
      .map(keyframeKey => Number.parseInt(keyframeKey, 10))
      .sort((a, b) => a - b)
...
function rmap(rows, iteratee) {
  return rows.map((row) => {
    ...
    if (typeof out !== 'object') {
      throw new TypeError('rmap iteratee must return an object')
    }
...
function rsmap(rows, iteratee) {
  return rows.map((row) => {
    ...
    if (typeof out !== 'string') {
      throw new TypeError('rmap iteratee must return a string')
    }
    return {
      text: out,

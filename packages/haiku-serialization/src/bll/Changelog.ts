...
export default class Changelog {
  cachedChangelog: any
  lastViewedChangelog: string | undefined
  ...
  readSingleChangelog(changelog: string) {
  ...
  readChangelogs() {
    const rawChangelogs = fs.readdirSync(this.changelogPath, 'utf8').filter(filename => filename === 'latest.json' || semver.gt(path.basename(filename, '.json'), this.lastViewedChangelog || '0.0.0')).sort((a, b) => {
      if (b === 'latest.json')
        return -1
      if (a === 'latest.json')
        return 1
  ...
  getChangelog() {
    return new Promise<any>((resolve, reject) => {
      if (this.cachedChangelog) {
        ...
        this.readChangelogs().then((changelogs) => {
          const latest = changelogs[changelogs.length - 1]
          const outputSections: Record<string, any[]> = {}
          for (const changelog of changelogs) {
            for (const section in changelog.sections) {
              outputSections[section] = [...changelog.sections[section], ...(outputSections[section] ? outputSections[section] : [])]
            }
          }

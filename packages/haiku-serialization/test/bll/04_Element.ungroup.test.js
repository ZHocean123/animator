...
const { getStub } = require("haiku-testing/lib/mock.js");
...
tape("Element.ungroup", suite => {
  suite.test("svg.01", test => {
    ...
    return Project.setup(
      ...
      envoyOptions,
      (err, project) => {
        return project.setCurrentActiveComponent(
          "main",
          { from: "test" },
          err => {
            if (err) {
          ...
          <svg width="200px" height="200px" viewBox="0 0 200 200" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
            ...
            return ac0.instantiateComponent(
              "designs/rects.svg",
              {},
              { from: "test" },
              (err, mana) => {
                ...
                test.is(
                  element.getUngroupables().length,
                  2,
                  "element has 2 ungroupable"
                );
                element.ungroup({ from: "test" }, () => {
                  ...
                  test.is(
                    ...
                    2,
                    "ungroup resulted in two children"
                  );
                  template.children.forEach((node, index) => {
                    test.is(
                      node.children[0].elementName,
                      "defs",
                      `defs were transcluded (${index})`
                    );
                    test.is(
                      node.children[0].children[0].elementName,
                      "path",
                      `path was transcluded in defs (${index})`
                    );
                    test.is(
                      node.children[1].elementName,
                      "g",
                      `shim group was created to shim layout (${index})`
                    );
                  });
                  test.deepEqual(
                    defaultTimeline[
                      `haiku:${template.children[0].children[1].attributes["haiku-id"]}`
                    ],
                    { "translation.x": { 0: { value: -100 } } },
                    "shim group was offset by the expected amount"
                  );
                  test.isNot(
                    template.children[0].children[0].children[0].attributes[
                      "haiku-id"
                    ],
                    template.children[1].children[0].children[0].attributes[
                      "haiku-id"
                    ],

...
Property.assignDOMSchemaProperties = function (out, element) {
    var schema = Property.BUILTIN_DOM_SCHEMAS[element.getSafeDomFriendlyName()] || {};
    for (var name_1 in schema) {
        var propertyGroup = null;
        var nameParts = name_1.split('.');
        var fallback = getFallback(element.getSafeDomFriendlyName(), name_1);
        propertyGroup = {
        ...
        if (propertyGroup) {
            ...
            out[name_1] = propertyGroup;
        }
    }
};
Property.doesPropertyGroupContainRotation = function (propertyGroup) {
    return (propertyGroup['rotation.x'] ||
...
Property.layoutSpecAsProperties = function (spec) {
    return {
        shown: spec.shown,
...
var NON_ROOT_ONLY = function (name, element) { return !element.isRootElement(); };
var ROOT_ONLY = function (name, element) { return element.isRootElement(); };
var NON_COMPONENT_ONLY = function (name, element) { return !element.isComponent(); };
var COMPONENT_ONLY = function (name, element) { return element.isComponent(); };
var ROOT_CHILD_ONLY = function (name, element, property, keyframes) { return element.isVisuallySelectable; };
var IF_EXPLICIT_OR_DEFINED = function (name, element, property, keyframes) { return (IF_EXPLICIT(name, element, property, keyframes) ||
    IF_DEFINED(name, element, property, keyframes)); };
var IF_EXPLICIT = function (name, element, property, keyframes) { return !!element._visibleProperties[name]; };
var IF_DEFINED = function (name, element, property, keyframes) { return (keyframes && Object.values(keyframes).some(function (keyframe) { return keyframe.edited; })); };
var IF_CHANGED_FROM_PREPOPULATED_VALUE = function (name, element, property, keyframes) { return wasChangedFromPrepopValue(name, keyframes); };
var IF_IN_SCHEMA = function (name, element) { return hasInSchema(element.getSafeDomFriendlyName(), name); };
var IF_TEXT_CONTENT_ENABLED = function (name, element, property, keyframes) { return (element.children.length < 1 ||
...
var wasChangedFromPrepopValue = function (name, keyframes) {
...
var hasInSchema = function (elementName, propertyName) { return (Property.BUILTIN_DOM_SCHEMAS[elementName] &&
    Property.BUILTIN_DOM_SCHEMAS[elementName][propertyName]); };
Property.areAnyKeyframesDefined = function (elementName, propertyName, keyframesObject) {
    var mss = Object.keys(keyframesObject);
...
Property.includeInJIT = function (name, element, property, keyframes) {
    return Property.includeInDisplay('jit', name, element, property, keyframes);
};
Property.includeInAddressables = function (name, element, property, keyframes) {
    return Property.includeInDisplay('add', name, element, property, keyframes);
};
Property.canHaveKeyframes = function (name, element, property, keyframes) {
...
Property.includeInDisplay = function (type, name, element, property, keyframes) {
    var rule = Property.DISPLAY_RULES[name];
    return rule && rule[type] && rule[type].every(function (test) { return test(name, element, property, keyframes); });
};
...
Property.hasColorPopup = function (propertyName) {
    return Property.WITH_COLOR_POPUP.has(propertyName);
};
var ROTATION_PI = Number(Math.PI.toFixed(2));
...
Property.hasRangePopup = function (propertyName) {
    return Property.WITH_RANGE_POPUP[propertyName];
};
Property.buildFilterObject = function (filtered, hostElement, propertyName, propertyObject) {
    ...
    if (Property.includeInAddressables(propertyName, hostElement, propertyObject, hostElement.getPropertyKeyframesObject(propertyName))) {
        filtered[propertyName] = propertyObject;
    }
};

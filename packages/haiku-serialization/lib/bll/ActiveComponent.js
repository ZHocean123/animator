...
var PlaybackFlag = require('@haiku/core/lib/HaikuTimeline').PlaybackFlag;
var _a = require('@haiku/core/lib/HaikuElement'), HAIKU_ID_ATTRIBUTE = _a.HAIKU_ID_ATTRIBUTE, HAIKU_LOCKED_ATTRIBUTE = _a.HAIKU_LOCKED_ATTRIBUTE, HAIKU_TITLE_ATTRIBUTE = _a.HAIKU_TITLE_ATTRIBUTE, HAIKU_VAR_ATTRIBUTE = _a.HAIKU_VAR_ATTRIBUTE;
var _b = require('@haiku/core/lib/HaikuComponent'), HaikuComponent = _b.default, clone = _b.clone;
var LAYOUT_3D_SCHEMA = require('@haiku/core/lib/HaikuComponent').LAYOUT_3D_SCHEMA;
var HaikuDOMAdapter = require('@haiku/core/lib/adapters/dom').default;
var getSortedKeyframes = require('@haiku/core/lib/helpers/KeyframeUtils').getSortedKeyframes;
var _c = require('@haiku/core/lib/helpers/interactionModes'), InteractionMode = _c.InteractionMode, isPreviewMode = _c.isPreviewMode;
var Layout3D = require('@haiku/core/lib/Layout3D');
...
var _d = require('haiku-common/lib/experiments'), Experiment = _d.Experiment, experimentIsEnabled = _d.experimentIsEnabled;
var Lock = require('./Lock');
var SustainedWarningChecker = require('haiku-common/lib/sustained-checker/SustainedWarningChecker').default;
...
var describeHotComponent = function (componentId, timelineName, timelineTime, propertyGroup) {
    ...
    for (var timelineName in keyframeUpdates) {
        for (var componentId in keyframeUpdates[timelineName]) {
            for (var propertyName in keyframeUpdates[timelineName][componentId]) {
                for (var keyframeMs in keyframeUpdates[timelineName][componentId][propertyName]) {
                    var hotComponent = describeHotComponent(componentId, timelineName, keyframeMs, [propertyName]);
                    if (hotComponent) {
...
var ActiveComponent = /** @class */ (function (_super) {
    ...
    function ActiveComponent(props, opts) {
        ...
        Element.on('update', function (element, what, metadata) {
            if (element.component === _this) {
                ...
                else if (what === 'jit-property-added' ||
        ...
        _this.commitAccumulatedKeyframeMovesDebounced = lodash.debounce(_this.commitAccumulatedKeyframeMoves.bind(_this), KEYFRAME_MOVE_DEBOUNCE_TIME);
        return _this;
    }
    ...
    ActiveComponent.prototype.findPropertyRowsByParentComponentId = function (parentHaikuId) {
    ...
    ActiveComponent.prototype.locateTemplateNodeByComponentId = function (componentId) {
    ...
    ActiveComponent.prototype.findTemplateNodeByComponentId = function (mana, componentId) {
        if (!mana) {
            return;
        }
        ...
        if (Array.isArray(mana.children)) {
            for (var i = 0; i < mana.children.length; i++) {
                var maybeChild = this.findTemplateNodeByComponentId(mana.children[i], componentId);
                if (maybeChild) {
                    return maybeChild;
                }
            }
    ...
    ActiveComponent.prototype.getCurrentTimelineTime = function () {
        ...
        var canonicalCoreTimeline = canonicalCoreInstance.getTimeline(this.getCurrentTimelineName());
    ...
    ActiveComponent.prototype.getRelpathWithRespectToProjectFromPathRelativeToUs = function (relpathRelativeToUs) {
        var abspathToGivenPath = path.normalize(path.join(this.getSceneCodeFolder(), relpathRelativeToUs));
        var relpathWithRespectToProject = abspathToGivenPath.replace(this.project.getFolder(), '').slice(1); // Remove leftover slash
    ...
    ActiveComponent.prototype.setAsCurrentActiveComponent = function (metadata, cb) {
    ...
    ActiveComponent.prototype.getFriendlySceneName = function (maybeProjectName) {
        ...
        if (snakename === DEFAULT_SCENE_NAME) {
            return "".concat(this.project.getFriendlyName(maybeProjectName), " (Main)");
        }
    ...
    ActiveComponent.prototype.getPropertyGroupValueFromPropertyKeys = function (componentId, timelineName, timelineTime, propertyKeys) {
        var groupValue = {};
        ...
        var cluster = bytecode.timelines[timelineName]["haiku:".concat(componentId)];
    ...
    ActiveComponent.prototype.htmlSnapshot = function (cb) {
        ...
        pretty(html).replace(/web\+haikuroot:\/\//g, ensureTrailingSlash(this.project.getFolder())));
    };
    ActiveComponent.prototype.setCurrentTimelineFrameValue = function (frame) {
        this.getCurrentTimeline().seek(frame, true);
    };
    ActiveComponent.prototype.setTimelineTimeValue = function (timelineTime, forceSeek) {
        if (forceSeek === void 0) { forceSeek = false; }
    ...
    ActiveComponent.prototype.setTitleForComponent = function (componentId, newTitle, metadata, cb) {
        var _this = this;
        this.project.updateHook('setTitleForComponent', this.getRelpath(), componentId, newTitle, metadata, function (fire) {
            return _this.performComponentWork(function (bytecode, mana, done) {
                var templateNode = _this.locateTemplateNodeByComponentId(componentId);
                if (!templateNode) {
                    return done(null, '', '');
                }
                if (newTitle) {
                ...
                return done(null, templateNode.attributes[HAIKU_TITLE_ATTRIBUTE], templateNode.attributes[HAIKU_TITLE_ATTRIBUTE]);
    ...
    ActiveComponent.prototype.setLockedStatusForComponent = function (componentId, locked, metadata, cb) {
        var _this = this;
        this.project.updateHook('setLockedStatusForComponent', this.getRelpath(), componentId, locked, metadata, function (fire) {
            return _this.performComponentWork(function (bytecode, mana, done) {
                var templateNode = _this.locateTemplateNodeByComponentId(componentId);
                if (!templateNode) {
                    return done(null, '', '');
     ...
     */
    ActiveComponent.prototype.handleElementSelected = function (componentId, metadata) {
        metadata.integrity = false;
        this.project.updateHook('selectElement', this.getRelpath(), componentId, metadata, function (fire) { return fire(); });
    };
    /**
     ...
     */
    ActiveComponent.prototype.handleElementUnselected = function (componentId, metadata) {
        metadata.integrity = false;
        this.project.updateHook('unselectElement', this.getRelpath(), componentId, metadata, function (fire) { return fire(); });
    };
    ActiveComponent.prototype.handleElementHovered = function (componentId, metadata) {
        metadata.integrity = false;
        this.project.updateHook('hoverElement', this.getRelpath(), componentId, metadata, function (fire) { return fire(); });
    };
    ActiveComponent.prototype.handleElementUnhovered = function (componentId, metadata) {
        metadata.integrity = false;
        this.project.updateHook('unhoverElement', this.getRelpath(), componentId, metadata, function (fire) { return fire(); });
    };
    ...
    ActiveComponent.prototype.selectElementWithinTime = function (waitTime, componentId, metadata, cb) {
        var _this = this;
        ...
        if (!element) {
            ...
            return setTimeout(function () {
                return _this.selectElementWithinTime(waitTime - SELECTION_PING_TIME, componentId, metadata, cb);
            }, SELECTION_PING_TIME);
        }
    ...
    ActiveComponent.prototype.selectAll = function (options, metadata, cb) {
        ...
        return Lock.request(Lock.LOCKS.ActiveComponentWork, false, function (release) {
            _this.getArtboard().getElement().children.forEach(function (element) {
                if (element.isLocked()) {
                    return;
                }
            ...
            release();
            _this.project.updateHook('selectAll', _this.getRelpath(), options, metadata, function (fire) { return fire(); });
            return cb();
        });
    };
    ActiveComponent.prototype.selectElement = function (componentId, metadata, cb) {
        return this.selectElementWithinTime(SELECTION_WAIT_TIME, componentId, metadata, function () {
            return cb(); // Must return or the plumbing action circuit never completes
        });
    };
    ActiveComponent.prototype.unselectElementWithinTime = function (waitTime, componentId, metadata, cb) {
        var _this = this;
        var element = Element.findByComponentAndHaikuId(this, componentId);
        if (!element) {
            if (waitTime <= 0) {
            ...
            return setTimeout(function () {
                return _this.unselectElementWithinTime(waitTime - SELECTION_PING_TIME, componentId, metadata, cb);
            }, SELECTION_PING_TIME);
        }
        element.unselect(metadata);
        return cb();
    };
    ActiveComponent.prototype.unselectElement = function (componentId, metadata, cb) {
        return this.unselectElementWithinTime(SELECTION_WAIT_TIME, componentId, metadata, function () {
            return cb(); // Must return or the plumbing action circuit never completes
        });
    };
    ...
    ActiveComponent.prototype.unhoverElement = function (componentId, metadata, cb) {
        var element = Element.findByComponentAndHaikuId(this, componentId);
        if (element) {
    ...
    ActiveComponent.prototype.getInsertionPointInfo = function (nonce) {
        ...
        var template = mana && Template.manaWithOnlyMinimalProps(mana, function () { return ({}); });
    ...
    ActiveComponent.prototype.doesMatchOrHostComponent = function (other, cb) {
        ...
        if (Template.normalizePath(other.getRelpath()) ===
            Template.normalizePath(this.getRelpath())) {
            return cb(null, true);
        }
        return cb(null, Bytecode.doesMatchOrHostBytecode(this.getReifiedBytecode(), other.getReifiedBytecode(), undefined));
    };
    /**
     ...
     * @param cb {Function}
     */
    ActiveComponent.prototype.instantiateReference = function (subcomponent, identifier, modpath, coords, overrides, metadata, cb) {
        var _this = this;
        return subcomponent.doesMatchOrHostComponent(this, function (err, answer) {
            ...
            return mod.moduleAsMana(_this.getRelpath(), identifier, title, function (err, manaForWrapperElement) {
                if (err) {
                    return cb(err);
                }
                if (!manaForWrapperElement) {
                    return cb(new Error("Module ".concat(fullpath, " could not be imported")));
                }
                _this.instantiateManaInBytecode(manaForWrapperElement, _this.getReifiedBytecode(), overrides, coords);
    ...
    ActiveComponent.prototype.fetchTimelinePropertyFromComponentElement = function (mana, propertyName) {
        if (!mana.elementName) {
            return;
        }
        ...
        return TimelineProperty.getComputedValue(mana.elementName.template.attributes[HAIKU_ID_ATTRIBUTE], mana.elementName.template.elementName, propertyName, this.getCurrentTimelineName(), this.getCurrentTimelineTime(), 0, mana.elementName, mana.__memory && mana.__memory.subcomponent, // can be undefined
        mana.__memory && mana.__memory.subcomponent && mana.__memory.subcomponent.state);
    };
    ActiveComponent.prototype.instantiateManaInBytecode = function (mana, bytecode, overrides, coords) {
        ...
        var timelines = Template.prepareManaAndBuildTimelinesObject(mana, hash, timelineName, timelineTime, { doHashWork: true });
        // Has to happen after the above stanza in case an id was generated
        var componentId = mana.attributes[HAIKU_ID_ATTRIBUTE];
        logger.info("[active component (".concat(this.project.getAlias(), ")] instantiatee (mana) ").concat(componentId, " via ").concat(hash));
        ...
        Bytecode.applyOverrides(overrides, timelines, timelineName, "haiku:".concat(componentId), timelineTime);
     ...
     * @param cb {Function}
     */
    ActiveComponent.prototype.instantiateMana = function (mana, bytecode, coords, metadata, cb) {
    ...
    ActiveComponent.prototype.createInTransitionInTimelineObject = function (timelineObj, propertyName, fromTime, fromValue, toTime, toValue, curveName) {
    ...
    ActiveComponent.prototype.mutateInstantiateeDisplaySettings = function (componentId, timelinesObject, timelineName, timelineTime, templateObject, maybeCoords) {
        ...
        var insertedTimeline = timelinesObject[this.getCurrentTimelineName()]["haiku:".concat(componentId)] || {};
        ...
        if (templateObject.elementName && typeof templateObject.elementName === 'object') {
            var sizeAbsoluteX = this.fetchTimelinePropertyFromComponentElement(templateObject, 'sizeAbsolute.x');
            if (sizeAbsoluteX) {
            ...
            var sizeAbsoluteY = this.fetchTimelinePropertyFromComponentElement(templateObject, 'sizeAbsolute.y');
            if (sizeAbsoluteY) {
        ...
        if (maybeCoords !== undefined && maybeCoords !== null) {
            var propertyGroup = {};
            var _a = this.getContextSizeActual(timelineName, timelineTime), width = _a.width, height = _a.height;
            ...
            TimelineProperty.addPropertyGroup(timelinesObject, timelineName, componentId, Element.safeElementName(templateObject), propertyGroup, timelineTime);
        }
    };
    /**
     * @method unconglomerateComponent
     */
    ActiveComponent.prototype.unconglomerateComponent = function (componentIds, name, size, translation, coords, propertiesSerial, options, metadata, cb) {
        var _this = this;
        if (options === void 0) { options = {}; }
        Lock.request(Lock.LOCKS.ActiveComponentWork, false, function (release) { return _this.project.updateHook('unconglomerateComponent', _this.getRelpath(), 
        ...
        componentIds, name, size, translation, coords, propertiesSerial, options, metadata, function (fire) {
            _this.fetchActiveBytecodeFile().updateInMemoryHotModule(_this.snapshots.pop(), function () {
     ...
     */
    ActiveComponent.prototype.conglomerateComponent = function (componentIds, name, size, translation, coords, propertiesSerial, options, metadata, cb) {
        var _this = this;
        if (options === void 0) { options = {}; }
        ...
        return Lock.request(Lock.LOCKS.ActiveComponentWork, false, function (release) {
            return _this.pushBytecodeSnapshot(function () { return _this.project.updateHook('conglomerateComponent', _this.getRelpath(), componentIds, name, size, translation, coords, Bytecode.serializeValue(properties), options, metadata, function (fire) {
                var finish = function (err, ac) {
                    if (err) {
                        release();
                        logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                        return cb(err);
                    }
                    return _this.reload({
                        hardReload: true,
                ...
                return _this.conglomerateComponentActual(componentIds, name, size, translation, coords, properties, options, metadata, finish);
            }); });
        });
    };
    ActiveComponent.prototype.conglomerateComponentActual = function (ids, name, size, translation, coords, properties, options, metadata, cb) {
        var _this = this;
        if (options === void 0) { options = {}; }
        var activeComponentToReturn;
        return this.performComponentWork(function (hostBytecode, hostTemplate, done) {
            return _this.project.upsertSceneByName(name, function (err, newActiveComponent) {
                if (err) {
                    return done(err);
                }
                ...
                newActiveComponent.upsertProperties(newBytecode, newBytecode.template.attributes[HAIKU_ID_ATTRIBUTE], newActiveComponent.getInstantiationTimelineName(), newActiveComponent.getInstantiationTimelineTime(), lodash.assign({
                ...
                ids.forEach(function (id) {
                    ...
                    var selector = Template.buildHaikuIdSelector(elementBytecode.template.attributes[HAIKU_ID_ATTRIBUTE]);
                    ...
                    for (var propertyName in elementOffset) {
                        ...
                        for (var keyframeMs in elementBytecode.timelines[timelineName][selector][propertyName]) {
                            var existingValue = elementBytecode.timelines[timelineName][selector][propertyName][keyframeMs].value || 0;
                            var existingCurve = elementBytecode.timelines[timelineName][selector][propertyName][keyframeMs].curve;
                ...
                return newActiveComponent.reload({
                    ...
                    var identifier = ModuleWrapper.modulePathToIdentifierName(relpath);
                    ...
                    return _this.instantiateReference(newActiveComponent, // subcomponent
                    identifier, relpath, coords, // "coords"/"maybeCoords"
                    ...
                    metadata, function (err) {
                        ...
                        var insertion = _this.getReifiedBytecode().template.children[0];
                        _this.upsertProperties(_this.getReifiedBytecode(), insertion.attributes[HAIKU_ID_ATTRIBUTE], _this.getInstantiationTimelineName(), 0, { playback: "loop" /* PlaybackFlag.LOOP */ }, 'merge');
                        return done();
                    });
                });
            });
    ...
    ActiveComponent.prototype.instantiateBytecode = function (incomingBytecode) {
        ...
        logger.info("[active component (".concat(this.project.getAlias(), ")] instantiatee (bytecode) ").concat(componentId, " via ").concat(hash));
     ...
     * @param cb {Function}
     */
    ActiveComponent.prototype.instantiateComponent = function (relpath, coords, metadata, cb) {
        var _this = this;
        return Lock.request(Lock.LOCKS.ActiveComponentWork, false, function (release) {
            return _this.project.updateHook('instantiateComponent', _this.getRelpath(), relpath, coords, metadata, function (fire) {
                ...
                var finish = function (err, manaForWrapperElement) {
                    ...
                    return _this.reload({
                        ...
                        return _this.selectElement(manaForWrapperElement.attributes[HAIKU_ID_ATTRIBUTE], metadata, function () { });
                    });
                };
                return _this.performComponentWork(function (bytecode, mana, done) {
                    ...
                    if (ModuleWrapper.doesRelpathLookLikeInstalledComponent(relpath)) {
                        ...
                        return _this.instantiateReference(installedComponent, installedComponent.getIdentifier(), relpath, coords, { 'origin.x': 0.5, 'origin.y': 0.5 }, metadata, done);
                    }
                    ...
                    if (ModuleWrapper.doesRelpathLookLikeLocalComponent(relpath)) {
                        return _this.project.findActiveComponentBySource(relpath, function (err, subcomponent) {
                            if (!err && subcomponent) {
                                ...
                                return subcomponent.moduleReload('basicReload', function () {
                                    ...
                                    var localComponentIdentifier = ModuleWrapper.modulePathToIdentifierName(relpath);
                                    return _this.instantiateReference(subcomponent, localComponentIdentifier, relpath, coords, { 'origin.x': 0.5, 'origin.y': 0.5 }, metadata, done);
                                });
                            }
                            return done(new Error("Cannot find component ".concat(relpath)));
                        });
                    }
                    if (ModuleWrapper.doesRelpathLookLikeSVGDesign(relpath)) {
                        return File.readMana(_this.project.getFolder(), relpath, function (err, mana) {
                            if (err) {
                                return done(err);
                            }
                            Template.fixManaSourceAttribute(mana, relpath); // Adds haiku-source="relpath_to_file_from_project_root"
                            return _this.instantiateMana(mana, bytecode, coords, metadata, done);
                        });
                    }
                    if (Asset.isImage(relpath)) {
                        ...
                        return imageComponent_1.queryImageSize(function (err, size) {
                            ...
                            return _this.instantiateReference(imageComponent_1, // subcomponent
    ...
    ActiveComponent.prototype.deleteComponents = function (componentIds, metadata, cb) {
        var _this = this;
        return Lock.request(Lock.LOCKS.ActiveComponentWork, false, function (release) {
            _this.project.updateHook('deleteComponents', _this.getRelpath(), componentIds, metadata, function (fire) {
                return _this.performComponentWork(function (bytecode, mana, done) {
    ...
    ActiveComponent.prototype.deleteElementImpl = function (mana, componentId) {
        Template.visitManaTree(mana, function (elementName, attributes, children, node, locator, parent, index) {
            if (!attributes) {
                return null;
            }
    ...
    ActiveComponent.prototype.mergePrimitiveWithOverrides = function (primitive, overrides, cb) {
        var _this = this;
        return this.performComponentWork(function (bytecode, template, done) {
            Template.visit((template), function (node) {
                ...
                var timelineObj = (bytecode.timelines &&
    ...
    ActiveComponent.prototype.removeChildContentFromBytecode = function (bytecode, mana) {
    ...
    ActiveComponent.prototype.findEquivalentNode = function (node, _a, template) {
        var index = _a.index, depth = _a.depth, address = _a.address;
        var foundNode;
        var ourDomId = node.attributes && node.attributes.id;
        Template.visit(template, function (desc, parent, theirIndex, theirDepth, theirAddress) {
    ...
    ActiveComponent.prototype.mergeRemovedOutputs = function (bytecode, subtemplate, removals) {
        // Nothing to do if there aren't any timelines to merge into
        if (!bytecode.timelines) {
            return;
        }
        for (var haikuId in removals) {
            var _a = removals[haikuId], treeInfo = _a.treeInfo, templateNode = _a.templateNode, timelines = _a.timelines;
            var equivalent = this.findEquivalentNode(templateNode, treeInfo, subtemplate);
            if (!equivalent) {
                continue;
            }
            var equivalentId = equivalent.attributes && equivalent.attributes[HAIKU_ID_ATTRIBUTE];
            if (!equivalentId) {
                continue;
            }
            ...
            for (var timelineName in bytecode.timelines) {
                ...
                for (var propertyName in timelines[timelineName]) {
                    for (var keyframeMs in timelines[timelineName][propertyName]) {
                        ...
                        var targetObj = bytecode.timelines[timelineName][equivalentSelector][propertyName][keyframeMs];
    ...
    ActiveComponent.prototype.mergeMana = function (existingBytecode, manaIncoming, index, _a) {
        var _this = this;
        var _b = _a.mergeRemovedOutputs, mergeRemovedOutputs = _b === void 0 ? true : _b;
        var numMatchingNodes = 0;
        var timelineName = this.getMergeDesignTimelineName();
        var timelineTime = this.getMergeDesignTimelineTime();
        Template.visitWithoutDescendingIntoSubcomponents(existingBytecode.template, function (existingNode) {
            ...
            var removedOutputs = _this.removeChildContentFromBytecode(existingBytecode, existingNode);
            var hash = _this.getInsertionPointInfo("".concat(index, "-").concat(numMatchingNodes++)).hash;
            var timelinesObject = Template.prepareManaAndBuildTimelinesObject(safeIncoming, hash, timelineName, timelineTime, {
                doHashWork: true,
            });
            var existingSelector = "haiku:".concat(existingNode.attributes[HAIKU_ID_ATTRIBUTE]);
            var incomingSelector = "haiku:".concat(safeIncoming.attributes[HAIKU_ID_ATTRIBUTE]);
            ...
            if (mergeRemovedOutputs) {
                _this.mergeRemovedOutputs(existingBytecode, existingNode, removedOutputs);
            }
        });
    };
    ...
    ActiveComponent.prototype.mergeDesignFilesImpl = function (designs, bytecode, _a, cb) {
        var _this = this;
        var _b = _a.mergeRemovedOutputs, mergeRemovedOutputs = _b === void 0 ? true : _b;
        ...
        Template.visitWithoutDescendingIntoSubcomponents(bytecode.template, function (existingNode) {
            // Only merge into any that match our source design path
            if (existingNode.attributes[HAIKU_SOURCE_ATTRIBUTE]) {
        ...
        return async.eachOfSeries(designsAsArray, function (relpath, index, next) {
            if (ModuleWrapper.doesRelpathLookLikeSVGDesign(relpath) && usedSources.has(path.posix.normalize(relpath))) {
                return File.readMana(_this.project.getFolder(), relpath, function (err, mana) {
                    ...
                    Template.fixManaSourceAttribute(mana, relpath); // Adds haiku-source="relpath_to_file_from_project_root"
                    _this.mergeMana(bytecode, mana, index, { mergeRemovedOutputs: mergeRemovedOutputs });
                    return next();
                });
            }
            return next();
     ...
     * @param cb {Function}
     */
    ActiveComponent.prototype.pasteThings = function (pasteablesSerial, options, metadata, cb) {
        var _this = this;
        var pasteables = pasteablesSerial.map(function (pasteableSerial) { return Bytecode.unserializeValue(pasteableSerial, function (ref) {
            return _this.evaluateReference(ref);
        }); });
        return Lock.request(Lock.LOCKS.ActiveComponentWork, false, function (release) {
            return _this.project.updateHook('pasteThings', _this.getRelpath(), pasteablesSerial, options, metadata, function (fire) {
                return _this.performComponentWork(function (bytecode, mana, done) {
                    var haikuIds = [];
                    return async.eachSeries(pasteables, function (pasteable, next) {
                        if (pasteable.kind === 'bytecode') {
                            // Handle specially if the pasted thing is a component
                            var nested_1 = (pasteable.data &&
                                pasteable.data.template &&
                                pasteable.data.template.elementName);
                            if (typeof nested_1 === 'object') {
                                var source = pasteable.data.template.attributes[HAIKU_SOURCE_ATTRIBUTE];
                                var identifier = pasteable.data.template.attributes[HAIKU_VAR_ATTRIBUTE];
                                var scenename = _this.project.relpathToSceneName(source);
                                nested_1.__reference = ModuleWrapper.buildReference(ModuleWrapper.REF_TYPES.COMPONENT, // type
                                Template.normalizePath("./".concat(_this.getRelpath())), // host
                                Template.normalizePathOfPossiblyExternalModule(source), identifier);
                                return _this.project.findOrCreateActiveComponent(scenename, function (err, ac) {
                                    if (err) {
                                        return next(err);
                                    }
                                    // We can't go further unless we actually have the reified bytecode
                                    return ac.moduleReload('basicReload', function () {
                                        ac.doesMatchOrHostComponent(_this, function (_, answer) {
                                            ...
                                            if (!answer) {
                                                ...
                                                lodash.assign(nested_1, ac.getReifiedBytecode());
                                                haikuIds.push(_this.pasteBytecodeImpl(bytecode, pasteable.data, options));
                                            }
                                            return next();
                                        });
                                    });
                        ...
                        logger.warn("[active component (".concat(_this.project.getAlias(), ")] cannot paste ").concat(pasteable.kind));
                        return next();
                    }, function (err) {
    ...
    ActiveComponent.prototype.pasteBytecodeImpl = function (ourBytecode, theirBytecode, _a) {
        var _b = _a.skipHashPadding, skipHashPadding = _b === void 0 ? false : _b;
        ...
        logger.info("[active component (".concat(this.project.getAlias(), ")] pastee (bytecode) ").concat(haikuId));
    ...
    ActiveComponent.prototype.splitSelectedKeyframes = function (metadata) {
        ...
        keyframes.forEach(function (keyframe) { return keyframe.removeCurve(metadata); });
    };
    ...
    ActiveComponent.prototype.joinSelectedKeyframes = function (curveName, metadata) {
        var keyframes = this.getSelectedKeyframes();
        keyframes.forEach(function (keyframe) {
    ...
    ActiveComponent.prototype.changeCurveOnSelectedKeyframes = function (curveName, metadata) {
        var keyframes = this.getSelectedKeyframes();
        keyframes.forEach(function (keyframe) {
            // Only keyframes that have a next keyframe should get the curve assigned,
            // otherwise you'll see a "surprise curve" if you add a next keyframe.
    ...
    ActiveComponent.prototype.getFirstSelectedCurve = function () {
        var keyframes = this.getSelectedKeyframes();
        var selectedKeyframeWithCurve = keyframes.find(function (keyframe) { return keyframe.isSelectedBody(); });
        return selectedKeyframeWithCurve ? selectedKeyframeWithCurve.getCurve() : null;
    };
    ActiveComponent.prototype.dragStartSelectedKeyframes = function (dragData, referenceKeyframe) {
        var keyframes = this.getSelectedKeyframes();
        if (referenceKeyframe && Keyframe.groupIsSingleTween(keyframes)) {
            referenceKeyframe.dragStart(dragData);
        }
        else {
            keyframes.forEach(function (keyframe) { return keyframe.dragStart(dragData); });
        }
    };
    ActiveComponent.prototype.dragStopSelectedKeyframes = function () {
        var keyframes = this.getSelectedKeyframes();
        keyframes.forEach(function (keyframe) { return keyframe.dragStop(); });
    ...
    ActiveComponent.prototype.dragSelectedKeyframes = function (pxpf, mspf, dragData, metadata, referenceKeyframe) {
        var keyframes = this.getSelectedKeyframes();
        if (referenceKeyframe && Keyframe.groupIsSingleTween(keyframes)) {
            referenceKeyframe.drag(pxpf, mspf, dragData, metadata);
        }
        else {
            keyframes.forEach(function (keyframe) { return keyframe.drag(pxpf, mspf, dragData, metadata); });
        }
    };
    // Returns true iff the element has any transitions or expressions.
    ActiveComponent.prototype.elementHasTransitionOrExpression = function (elementId) {
        var bytecode = this.getReifiedBytecode();
        ...
        if (componentId in bytecode.timelines[timelineName]) {
            ...
            var _loop_1 = function (propertyName) {
                ...
                if (propertyTimeline instanceof Object) {
                    ...
                    var values_1 = keys.map(function (key) { return propertyTimeline[key].value; });
                    if (keys.length > 1) {
                        if (values_1.some(function (value) { return value !== values_1[0]; })) {
                            return { value: true };
                        }
                    }
                    if (values_1.some(function (value) { return value instanceof Function; })) {
                        return { value: true };
                    }
                }
            };
    ...
    ActiveComponent.prototype.snapshotKeyframeUpdates = function (keyframeUpdates) {
        var bytecode = this.getReifiedBytecode();
        var updates = {};
        for (var timelineName in keyframeUpdates) {
            ...
            for (var componentId in keyframeUpdates[timelineName]) {
                ...
                for (var propertyName in keyframeUpdates[timelineName][componentId]) {
                    ...
                    for (var keyframeMs in keyframeUpdates[timelineName][componentId][propertyName]) {
                        if (!bytecode.timelines[timelineName] ||
                            ...
                            if (Number(keyframeMs) === 0) {
                                ...
                                updates[timelineName][componentId][propertyName][keyframeMs] = {
                                    value: TimelineProperty.getFallbackValue(elementName, propertyName),
                                };
                            }
                            else {
                        ...
                        var keyfVal = (typeof bytecode.timelines[timelineName][selector][propertyName][keyframeMs].value === 'function')
    ...
    ActiveComponent.prototype.gatherKeyframeMoves = function (componentId, timelineName, propertyNames) {
    ...
    ActiveComponent.prototype.snapshotKeyframeMoves = function (keyframeMovesDescriptor) {
        var moves = {};
        for (var timelineName in keyframeMovesDescriptor) {
            ...
            for (var componentId in keyframeMovesDescriptor[timelineName]) {
                ...
                var propertyNames = Object.keys(keyframeMovesDescriptor[timelineName][componentId]);
                var keyframesObj = this.getKeyframesObjectForPropertyNames(timelineName, componentId, propertyNames);
                for (var propertyName in keyframesObj) {
                    ...
                    for (var keyframeMs in propertyObj) {
                        ...
                        var keyfVal = (typeof keyfObj.value === 'function')
                            ? keyfObj.value
    ...
    ActiveComponent.prototype.commitAccumulatedKeyframeMoves = function () {
        this.moveKeyframes(Keyframe.buildKeyframeMoves({ component: this }, true), this.project.getMetadata(), function () { });
    };
    ...
    /** ------------ */
    /** ------------ */
    ActiveComponent.prototype.reload = function (reloadOptions, instanceConfig, cb) {
        var _this = this;
        ...
        return Lock.request(Lock.LOCKS.ActiveComponentReload, false, function (release) {
            var finish = function (err) {
                ...
                _this.emit('update', 'reloaded', (reloadOptions.hardReload) ? 'hard' : 'soft');
                return cb();
            };
            return runReload(finish);
        });
    };
    ActiveComponent.prototype.softReload = function (reloadOptions, instanceConfig, cb) {
    ...
    ActiveComponent.prototype.hardReload = function (reloadOptions, instanceConfig, finish) {
        var _this = this;
        ...
        return async.series([
            ...
            function (cb) {
                ...
                return _this.moduleCreate(reloadOptions.moduleReloadMethod, instanceConfig, cb);
            },
            function (cb) {
            ...
            function (cb) {
                ...
                if (_this.$instance) {
                    ...
                    var timeline = _this.$instance.getTimeline(_this.getCurrentTimelineName());
                    if (timeline) {
                        timeline.setPlaying(true);
                    }
                }
                // Solely used to allow glass to update internally when the authoritative frame changes
                _this.project.emit('change-authoritative-frame', Math.round(timelineTimeBeforeReload / _this.getCurrentMspf()));
                return cb();
            },
        ], finish);
    };
    ActiveComponent.prototype.destroy = function (cleanup) {
        ...
        for (var _i = 0, _a = [MountElement, Artboard, SelectionMarquee, Timeline, Keyframe, Row, Element, ElementSelectionProxy]; _i < _a.length; _i++) {
            var klass = _a[_i];
            klass.where({ component: this }).forEach(function (instance) { return instance.destroy(); });
        }
        _super.prototype.destroy.call(this);
    };
    ActiveComponent.prototype.moduleReload = function (moduleReloadMethod, cb) {
        if (moduleReloadMethod === void 0) { moduleReloadMethod = 'basicReload'; }
    ...
    ActiveComponent.prototype.doesManageCoreInstance = function (instance) {
        ...
        return (path.normalize(instance.getBytecodeRelpath()) ===
            path.normalize(this.getRelpath()));
    };
    ActiveComponent.prototype.moduleCreate = function (moduleReloadMethod, instanceConfig, cb) {
        var _this = this;
        ...
        return this.moduleReload(moduleReloadMethod, function (err) {
            ...
            _this.sustainedWarningsChecker = new SustainedWarningChecker(_this.$instance);
            // Use debounce to emit event to trigger sustained warnings check on haiku-creator
            _this.emitDebouncedCheckSustainedWarning = lodash.debounce(function () {
    ...
    ActiveComponent.prototype.moduleFindOrCreate = function (moduleReloadMethod, instanceConfig, cb) {
        if (this.$instance) {
            return cb();
        }
    ...
    ActiveComponent.prototype.createInstance = function (bytecode, config) {
        ...
        var createdHaikuCoreComponent = factory(this.getMount().$el(), lodash.merge({}, {
     ...
     * by the Glass.
     */
    ActiveComponent.prototype.mountApplication = function ($el, instanceConfig, cb) {
        var _this = this;
    ...
    ActiveComponent.prototype.fetchRootElement = function () {
        ...
        return Element.upsertElementFromVirtualElement(this, // component
        staticTemplateNode, null, // parent element
        0, // index in parent
        '0');
    };
    ActiveComponent.prototype.pushBytecodeSnapshot = function (done) {
        ...
        this.snapshots.push(Bytecode.snapshot(this.fetchActiveBytecodeFile().getReifiedDecycledBytecode({ suppressSubcomponents: false })));
        done();
    };
    ActiveComponent.prototype.popBytecodeSnapshot = function (metadata, cb) {
        var _this = this;
        return this.project.updateHook('popBytecodeSnapshot', this.getRelpath(), metadata, function (fire) {
            ...
            // one.
            _this.snapshots.splice(_this.snapshots.length - 2, 1)[0], function () {
                _this.moduleSync(function () {
                    fire();
                    return cb();
                });
    ...
    ActiveComponent.prototype.rehydrate = function (options) {
        ...
        Keyframe.where({ component: this }).forEach(function (keyframe) { return keyframe.mark(); });
        Row.where({ component: this }).forEach(function (row) { return row.mark(); });
        ...
        Row.where({ component: this }).forEach(function (row) { return row.sweep(); });
        Keyframe.where({ component: this }).forEach(function (keyframe) { return keyframe.sweep(); });
    ...
    ActiveComponent.prototype.upsertProperties = function (bytecode, componentId, timelineName, timelineTime, propertiesToMerge, strategy) {
        return Bytecode.upsertPropertyValue(bytecode, componentId, timelineName, timelineTime, propertiesToMerge, strategy);
    };
    ...
    ActiveComponent.prototype.getDeclaredPropertyValue = function (componentId, timelineName, timelineTime, propertyName) {
        var bytecode = this.getReifiedBytecode();
        var propertyValue = Template.getPropertyValue(bytecode, componentId, timelineName, timelineTime, propertyName);
        ...
        if (propertyValue === undefined || propertyValue === null) {
            var elementName = this.getElementNameOfComponentId(componentId);
            propertyValue = TimelineProperty.getFallbackValue(elementName, propertyName);
        }
        return propertyValue;
    };
    ActiveComponent.prototype.getDeclaredPropertyValues = function (componentId, timelineName, timelineTime, propertyNames) {
        var _this = this;
        var out = {};
        propertyNames.forEach(function (propertyName) {
            out[propertyName] = _this.getDeclaredPropertyValue(componentId, timelineName, timelineTime, propertyName);
        });
        return out;
    };
    ...
    ActiveComponent.prototype.getComputedPropertyValue = function (template, componentId, timelineName, timelineTime, propertyName, fallbackValue) {
        var bytecode = this.getReifiedBytecode();
    ...
    ActiveComponent.prototype.getContextSizeActual = function (timelineName, timelineTime) {
        ...
        var host = this.$instance;
        var states = (host && host.getStates()) || {};
        var contextWidth = TimelineProperty.getComputedValue(contextHaikuId, contextElementName, 'sizeAbsolute.x', timelineName || DEFAULT_TIMELINE_NAME, timelineTime || DEFAULT_TIMELINE_TIME, 0, bytecode, host, states);
        var contextHeight = TimelineProperty.getComputedValue(contextHaikuId, contextElementName, 'sizeAbsolute.y', timelineName || DEFAULT_TIMELINE_NAME, timelineTime || DEFAULT_TIMELINE_TIME, 0, bytecode, host, states);
    ...
    ActiveComponent.prototype.focusSelectNext = function (navDir, doFocus, metadata) {
    ...
    ActiveComponent.prototype.getDisplayableRowsGroupedByElementInZOrder = function () {
        var _this = this;
        var stack = this.getRawStackingInfo(this.getInstantiationTimelineName(), this.getInstantiationTimelineTime()).reverse();
        var root = this.fetchRootElement();
        ...
        ].concat(stack.reduce(function (acc, _a) {
    ...
    ActiveComponent.prototype.checkIfSelectedKeyframesAreMovableToZero = function () {
        ...
        var notMovable = selectedKeyframes.findIndex(function (keyframe) { return !(keyframe.prev() && keyframe.prev().origMs === 0); });
        return notMovable === -1;
    };
    ...
    ActiveComponent.prototype.performComponentWork = function (worker, cb) {
        ...
        this.sleepComponentsOn();
        return Lock.request(Lock.LOCKS.FilePerformComponentWork, false, function (release) {
            var finish = function (err) {
                var result = [];
    ...
    ActiveComponent.prototype.performComponentTimelinesWork = function (worker, finish) {
    ...
    ActiveComponent.prototype.getKeyframeValue = function (componentId, timelineName, timelineTime, propertyName) {
        var bytecode = this.getReifiedBytecode();
    ...
    ActiveComponent.prototype.getKeyframeCurve = function (componentId, timelineName, timelineTime, propertyName) {
        var bytecode = this.getReifiedBytecode();
        var selector = "haiku:".concat(componentId);
        return (bytecode &&
            bytecode.timelines &&
    ...
    ActiveComponent.prototype.getElementNameOfComponentId = function (componentId) {
        var element = this.findTemplateNodeByComponentId(this.getReifiedBytecode().template, componentId);
        return element && element.elementName;
    };
    ActiveComponent.prototype.getSafeElementNameOfComponentId = function (componentId) {
        var element = this.findTemplateNodeByComponentId(this.getReifiedBytecode().template, componentId);
    ...
    ActiveComponent.prototype.getRawStackingInfo = function (timelineName, timelineTime) {
        var bytecode = this.getReifiedBytecode();
        return Template.getStackingInfo(bytecode, bytecode.template, timelineName, timelineTime);
    };
    ActiveComponent.prototype.setZIndicesForStackingInfo = function (bytecode, timelineName, timelineTime, stackingInfo) {
        var _this = this;
        // If we received items out of order, fix their z-indexes.
        stackingInfo
            .forEach(function (_a, arrayIndex) {
            var haikuId = _a.haikuId;
            _this.upsertProperties(bytecode, haikuId, timelineName, timelineTime, {
                'style.zIndex': arrayIndex + 1,
            }, 'merge');
        });
    };
    ActiveComponent.prototype.grabStackObjectFromStackingInfo = function (stackingInfo, componentId) {
        for (var index = stackingInfo.length - 1; index >= 0; index--) {
            if (stackingInfo[index].haikuId === componentId) {
                return { ourStackObject: stackingInfo.splice(index, 1)[0], index: index };
            }
        }
    };
    /**
     ...
     */
    ActiveComponent.prototype.writeMetadata = function (bytecodeMetadata, metadata, cb) {
        var _this = this;
        return this.project.updateHook('writeMetadata', this.getRelpath(), bytecodeMetadata, metadata, function (fire) {
            return _this.performComponentWork(function (bytecode, mana, done) {
                Bytecode.writeMetadata(bytecode, lodash.assign({}, bytecodeMetadata, { title: _this.getTitle() }));
                done();
            }, function () {
                fire();
                cb();
     ...
     */
    ActiveComponent.prototype.batchUpsertEventHandlers = function (selectorName, eventsSerial, metadata, cb) {
        var _this = this;
        var events = Bytecode.unserializeValue(eventsSerial, function (ref) {
            return _this.evaluateReference(ref);
        });
        return this.project.updateHook('batchUpsertEventHandlers', this.getRelpath(), selectorName, Bytecode.serializeValue(events), metadata, function (fire) {
            return _this.batchUpsertEventHandlersActual(selectorName, events, function (err) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
                return _this.reload({
                    ...
                    fire();
                    _this.project.broadcastPayload({ name: 'event-handlers-updated' });
                    return cb();
                });
            });
        });
    };
    ActiveComponent.prototype.batchUpsertEventHandlersActual = function (selectorName, serializedEvents, cb) {
        return this.performComponentWork(function (bytecode, mana, done) {
     ...
     */
    ActiveComponent.prototype.changeKeyframeValue = function (componentId, timelineName, propertyName, keyframeMs, newValueSerial, metadata, cb) {
        var _this = this;
        var newValue = Bytecode.unserializeValue(newValueSerial, function (ref) {
            return _this.evaluateReference(ref);
        });
        return this.project.updateHook('changeKeyframeValue', this.getRelpath(), componentId, timelineName, propertyName, keyframeMs, Bytecode.serializeValue(newValue), metadata, function (fire) {
            return _this.changeKeyframeValueActual(componentId, timelineName, propertyName, keyframeMs, newValue, metadata, function (err) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
    ...
    ActiveComponent.prototype.changeKeyframeValueActual = function (componentId, timelineName, propertyName, keyframeMs, newValue, metadata, cb) {
        return this.performComponentWork(function (bytecode, mana, done) {
     ...
     */
    ActiveComponent.prototype.changeSegmentCurve = function (componentId, timelineName, propertyName, keyframeMs, newCurveSerial, metadata, cb) {
        var _this = this;
        var newCurve = Bytecode.unserializeValue(newCurveSerial, function (ref) {
            return _this.evaluateReference(ref);
        });
        return this.project.updateHook('changeSegmentCurve', this.getRelpath(), componentId, timelineName, propertyName, keyframeMs, Bytecode.serializeValue(newCurve), metadata, function (fire) {
            return _this.changeSegmentCurveActual(componentId, timelineName, propertyName, keyframeMs, newCurve, metadata, function (err) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
    ...
    ActiveComponent.prototype.changeSegmentCurveActual = function (componentId, timelineName, propertyName, keyframeMs, newCurve, metadata, cb) {
        return this.performComponentWork(function (bytecode, mana, done) {
     ...
     */
    ActiveComponent.prototype.joinKeyframes = function (componentId, timelineName, elementName, propertyName, keyframeMsLeft, keyframeMsRight, newCurveSerial, metadata, cb) {
        var _this = this;
        var newCurve = Bytecode.unserializeValue(newCurveSerial, function (ref) {
            return _this.evaluateReference(ref);
        });
        return this.project.updateHook('joinKeyframes', this.getRelpath(), componentId, timelineName, elementName, propertyName, keyframeMsLeft, keyframeMsRight, Bytecode.serializeValue(newCurve), metadata, function (fire) {
            return _this.joinKeyframesActual(componentId, timelineName, elementName, propertyName, keyframeMsLeft, keyframeMsRight, newCurve, metadata, function (err) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
    ...
    ActiveComponent.prototype.joinKeyframesActual = function (componentId, timelineName, elementName, propertyName, keyframeMsLeft, keyframeMsRight, newCurve, metadata, cb) {
        return this.performComponentWork(function (bytecode, mana, done) {
     ...
     * @method splitSegment
     */
    ActiveComponent.prototype.splitSegment = function (componentId, timelineName, elementName, propertyName, keyframeMs, metadata, cb) {
        var _this = this;
        return this.project.updateHook('splitSegment', this.getRelpath(), componentId, timelineName, elementName, propertyName, keyframeMs, metadata, function (fire) {
            return _this.splitSegmentActual(componentId, timelineName, elementName, propertyName, keyframeMs, metadata, function (err) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
    ...
    ActiveComponent.prototype.splitSegmentActual = function (componentId, timelineName, elementName, propertyName, keyframeMs, metadata, cb) {
        return this.performComponentWork(function (bytecode, mana, done) {
            Bytecode.splitSegment(bytecode, componentId, timelineName, elementName, propertyName, keyframeMs);
            done();
        }, cb);
    };
    ActiveComponent.prototype.getKeyframesObjectForPropertyNames = function (timelineName, componentId, propertyNames) {
        var bytecode = this.getReifiedBytecode() || {};
    ...
    ActiveComponent.prototype.ensureZerothKeyframe = function (bytecode, timelineName, componentId, propertyName, fallbackToInitialKeyframeIfProvided) {
        var _this = this;
        if (fallbackToInitialKeyframeIfProvided === void 0) { fallbackToInitialKeyframeIfProvided = true; }
        var selector = "haiku:".concat(componentId);
        if (!bytecode.timelines[timelineName]) {
            bytecode.timelines[timelineName] = {};
        }
        ...
        var initialKeyframeObj = (initialKeyframeMs !== undefined)
        ...
        if (descriptor[0].value === undefined) {
            if (fallbackToInitialKeyframeIfProvided && initialKeyframeObj) {
                descriptor[0].value = Bytecode.unserializeValue(initialKeyframeObj.value, function (ref) { return _this.evaluateReference(ref); });
            }
            else {
                // Otherwise, use the fallback if we have no next keyframe defined
                var declaredValue = this.getDeclaredPropertyValue(componentId, timelineName, 0, propertyName);
                descriptor[0].value = Bytecode.unserializeValue(declaredValue, function (ref) { return _this.evaluateReference(ref); });
            }
        }
        if (descriptor[0].value === undefined) {
     ...
     */
    ActiveComponent.prototype.moveKeyframes = function (keyframeMovesSerial, metadata, cb) {
        var _this = this;
        if (Object.keys(keyframeMovesSerial).length < 1) {
            return cb();
        }
        var keyframeMoves = Bytecode.unserializeValue(keyframeMovesSerial, function (ref) {
            return _this.evaluateReference(ref);
        });
        return this.project.updateHook('moveKeyframes', this.getRelpath(), Bytecode.serializeValue(keyframeMoves), metadata, function (fire) {
            return _this.moveKeyframesActual(keyframeMoves, metadata, function (err) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
                        ...
                        for (var timelineName in keyframeMoves) {
                            for (var componentId in keyframeMoves[timelineName]) {
                                ...
                                for (var propertyName in keyframeMoves[timelineName][componentId]) {
                                    ...
                                    row.getKeyframes().forEach(function (keyframe) { return keyframe.updateOwnMetadata(); });
                                    row.rehydrate();
                                }
                            }
                        }
    ...
    ActiveComponent.prototype.moveKeyframesActual = function (keyframeMoves, metadata, cb) {
        var _this = this;
        return this.performComponentWork(function (bytecode, mana, done) {
            ...
            for (var timelineName in keyframeMoves) {
                for (var componentId in keyframeMoves[timelineName]) {
                    for (var propertyName in keyframeMoves[timelineName][componentId]) {
                        _this.ensureZerothKeyframe(bytecode, timelineName, componentId, propertyName, true);
                    }
                }
            }
     ...
     */
    ActiveComponent.prototype.updateKeyframes = function (keyframeUpdatesSerial, options, metadata, cb) {
        var _this = this;
        var keyframeUpdates = Bytecode.unserializeValue(keyframeUpdatesSerial, function (ref) {
            return _this.evaluateReference(ref);
        });
        return this.project.updateHook('updateKeyframes', this.getRelpath(), Bytecode.serializeValue(keyframeUpdates), options, metadata, function (fire) {
            var unlockedDesigns = {};
            if (options.setElementLockStatus) {
                for (var elID in options.setElementLockStatus) {
                    var node = _this.findTemplateNodeByComponentId(_this.getReifiedBytecode().template, elID);
            ...
            return _this.updateKeyframesActual(keyframeUpdates, { unlockedDesigns: unlockedDesigns }, metadata, function (err) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
                        ...
                        var _loop_2 = function (timelineName) {
                            var _loop_3 = function (componentId) {
                                ...
                                if (element) {
                                    ...
                                    Row.where({ component: _this, element: element }).forEach(function (row) {
    ...
    ActiveComponent.prototype.updateKeyframesActual = function (keyframeUpdates, _a, metadata, cb) {
        var _this = this;
        ...
        return this.performComponentWork(function (bytecode, mana, done) {
            for (var timelineName in keyframeUpdates) {
                ...
                for (var componentId in keyframeUpdates[timelineName]) {
                    ...
                    for (var propertyName in keyframeUpdates[timelineName][componentId]) {
                        ...
                        for (var keyframeMs in keyframeUpdates[timelineName][componentId][propertyName]) {
                            var propertyObj = keyframeUpdates[timelineName][componentId][propertyName][keyframeMs];
                            if (propertyObj === null) {
                            ...
                            var keyfVal = (typeof propertyObj.value === 'function')
                                ? propertyObj.value
                            ...
                            _this.ensureZerothKeyframe(bytecode, timelineName, componentId, propertyName, false);
                            if (experimentIsEnabled(Experiment.AutoTweenNewKeyframes)) {
                                Bytecode.addDefaultCurveIfNecessary(bytecode, timelineName, selector, keyframeMs, propertyName, componentId, _this.getElementNameOfComponentId(componentId));
                            }
                        }
                    }
                }
            ...
            // Clear timeline caches; the max frame might have changed.
            Timeline.clearCaches();
            _this.mergeDesignFilesImpl(unlockedDesigns, bytecode, { mergeRemovedOutputs: false }, done);
        }, cb);
    };
    ActiveComponent.prototype.updateTypesActual = function (typeUpdates, metadata, cb) {
        var _this = this;
        return this.performComponentWork(function (bytecode, mana, done) {
    ...
    ActiveComponent.prototype.updateKeyframesAndTypes = function (keyframeUpdatesSerial, typeUpdates, options, metadata, cb) {
        var _this = this;
        var keyframeUpdates = Bytecode.unserializeValue(keyframeUpdatesSerial, function (ref) {
            return _this.evaluateReference(ref);
        });
        return this.project.updateHook('updateKeyframesAndTypes', this.getRelpath(), Bytecode.serializeValue(keyframeUpdates), typeUpdates, options, metadata, function (fire) {
            var unlockedDesigns = {};
            if (options.setElementLockStatus) {
                for (var elID in options.setElementLockStatus) {
                    var node = _this.findTemplateNodeByComponentId(_this.getReifiedBytecode().template, elID);
            ...
            return _this.updateKeyframesActual(keyframeUpdates, { unlockedDesigns: unlockedDesigns }, metadata, function (err) {
                ...
                return _this.updateTypesActual(typeUpdates, metadata, function (err) {
                    if (err) {
                        logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                        return cb(err);
                    }
     ...
     */
    ActiveComponent.prototype.createKeyframe = function (componentId, timelineName, elementName, propertyName, keyframeStartMs, keyframeValueSerial, keyframeCurveSerial, keyframeEndMs, keyframeEndValueSerial, options, metadata, cb) {
        var _this = this;
        var keyframeValue = Bytecode.unserializeValue(keyframeValueSerial, function (ref) {
            return _this.evaluateReference(ref);
        });
        var keyframeCurve = Bytecode.unserializeValue(keyframeCurveSerial, function (ref) {
            return _this.evaluateReference(ref);
        });
        var keyframeEndValue = Bytecode.unserializeValue(keyframeEndValueSerial, function (ref) {
            return _this.evaluateReference(ref);
        });
        var element = this.findElementByComponentId(componentId);
        var actualKeyframeStartMs = element && !Property.canHaveKeyframes(propertyName, element) ? 0 : keyframeStartMs;
        return this.project.updateHook('createKeyframe', this.getRelpath(), componentId, timelineName, elementName, propertyName, actualKeyframeStartMs, Bytecode.serializeValue(keyframeValue), Bytecode.serializeValue(keyframeCurve), keyframeEndMs, Bytecode.serializeValue(keyframeEndValue), options, metadata, function (fire) {
            var unlockedDesigns = {};
            ...
            return _this.createKeyframeActual(componentId, timelineName, elementName, propertyName, actualKeyframeStartMs, keyframeValue, keyframeCurve, keyframeEndMs, keyframeEndValue, metadata, function (err) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
    ...
    ActiveComponent.prototype.createKeyframeActual = function (componentId, timelineName, elementName, propertyName, keyframeStartMs, keyframeValue, keyframeCurve, keyframeEndMs, keyframeEndValue, metadata, cb) {
        var _this = this;
        return this.performComponentWork(function (bytecode, mana, done) {
            var host = _this.$instance;
            var states = (host && host.getStates()) || {};
            Bytecode.createKeyframe(bytecode, componentId, timelineName, elementName, propertyName, keyframeStartMs, keyframeValue, keyframeCurve, keyframeEndMs, keyframeEndValue, host, states);
            _this.ensureZerothKeyframe(bytecode, timelineName, componentId, propertyName, false);
            if (experimentIsEnabled(Experiment.AutoTweenNewKeyframes)) {
                Bytecode.addDefaultCurveIfNecessary(bytecode, timelineName, Template.buildHaikuIdSelector(componentId), keyframeStartMs, propertyName, componentId, elementName);
            }
            done();
        }, cb);
    };
     ...
     */
    ActiveComponent.prototype.deleteKeyframe = function (componentId, timelineName, propertyName, keyframeMs, metadata, cb) {
        var _this = this;
        return this.project.updateHook('deleteKeyframe', this.getRelpath(), componentId, timelineName, propertyName, keyframeMs, metadata, function (fire) {
            return _this.deleteKeyframeActual(componentId, timelineName, propertyName, keyframeMs, metadata, function (err) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
    ...
    ActiveComponent.prototype.deleteKeyframeActual = function (componentId, timelineName, propertyName, keyframeMs, metadata, cb) {
        var _this = this;
        return this.performComponentWork(function (bytecode, mana, done) {
     ...
     */
    ActiveComponent.prototype.groupElements = function (componentIds, groupMana, coords, metadata, cb) {
        var _this = this;
        return this.project.updateHook('groupElements', this.getRelpath(), componentIds, groupMana, coords, metadata, function (fire) {
            return _this.groupElementsActual(componentIds, groupMana, coords, metadata, function (err, groupComponentId) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
                return _this.reload({
                    ...
                    _this.findElementByComponentId(groupComponentId).select(metadata);
                    return cb();
                });
            });
        });
    };
    ActiveComponent.prototype.groupElementsActual = function (componentIds, groupManaIn, coords, metadata, cb) {
        var _this = this;
        // Make a copy so that we don't have to decycle.
        var groupMana = lodash.cloneDeep(groupManaIn);
        var originalTimeline = this.getTimelineDescriptor(this.getCurrentTimelineName());
        return this.performComponentWork(function (bytecode, mana, done) {
            var timelineName = _this.getInstantiationTimelineName();
            var timelineTime = _this.getInstantiationTimelineTime();
            var groupComponentId = _this.instantiateManaInBytecode(groupMana, bytecode, {}, coords);
            var nodesToRegroup = [];
            var _loop_4 = function (i) {
                ...
                if (componentIds.indexOf(node.attributes[HAIKU_ID_ATTRIBUTE]) !== -1) {
                    var timelineSelector = "haiku:".concat(node.attributes[HAIKU_ID_ATTRIBUTE]);
                    ...
                    var propertyGroup = Object.keys(originalTimeline[timelineSelector]).reduce(function (accumulator, propertyName) {
                        if (LAYOUT_3D_SCHEMA[propertyName]) {
                            accumulator[propertyName] = {
                                0: {
                                    value: _this.getComputedPropertyValue(mana, node.attributes[HAIKU_ID_ATTRIBUTE], timelineName, _this.getCurrentTimelineTime(), propertyName, undefined),
                                },
                            };
                        }
                        return accumulator;
            ...
            var stackingInfo = Template.getStackingInfo(bytecode, mana, timelineName, timelineTime);
            var stackObject = _this.grabStackObjectFromStackingInfo(stackingInfo, groupComponentId);
            ...
            if (ourStackObject) {
                ...
                logger.warn("[active component] stack object missing at ".concat(timelineName, " ").concat(timelineTime));
            }
            _this.setZIndicesForStackingInfo(bytecode, timelineName, timelineTime, stackingInfo);
     ...
     */
    ActiveComponent.prototype.ungroupElements = function (componentId, nodes, metadata, cb) {
        var _this = this;
        return this.project.updateHook('ungroupElements', this.getRelpath(), componentId, nodes, metadata, function (fire) {
            var clonedNodes = lodash.cloneDeep(nodes);
            return _this.ungroupElementsActual(componentId, clonedNodes, metadata, function (err, ungroupedComponentIds) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
    ...
    ActiveComponent.prototype.ungroupElementsActual = function (componentId, nodes, metadata, cb) {
        var _this = this;
        return this.performComponentWork(function (bytecode, mana, done) {
            // `nodes` is an array of clean mana we can instantiate as-is.
            var updatedComponentIds = nodes.map(function (node) {
                var componentId = _this.instantiateManaInBytecode(node, bytecode, {}, undefined);
                Template.visitManaTree(node, function (elementName, attributes, children, componentMana) {
                    ...
                    if (attributes && attributes['haiku-transclude']) {
                        var originalComponent = _this.getTemplateNodesByComponentId()[attributes['haiku-transclude']];
     ...
     */
    ActiveComponent.prototype.upsertStateValue = function (stateName, stateDescriptorSerial, metadata, cb) {
        var _this = this;
        var stateDescriptor = Bytecode.unserializeValue(stateDescriptorSerial, function (ref) {
            return _this.evaluateReference(ref);
        });
        return this.project.updateHook('upsertStateValue', this.getRelpath(), stateName, Bytecode.serializeValue(stateDescriptor), metadata, function (fire) {
            stateDescriptor.edited = true;
            return _this.upsertStateValueActual(stateName, stateDescriptor, metadata, function (err) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
    ...
    ActiveComponent.prototype.upsertStateValueActual = function (stateName, stateDescriptor, metadata, cb) {
        return this.performComponentWork(function (bytecode, mana, done) {
     ...
     */
    ActiveComponent.prototype.deleteStateValue = function (stateName, metadata, cb) {
        var _this = this;
        return this.project.updateHook('deleteStateValue', this.getRelpath(), stateName, metadata, function (fire) {
    ...
    ActiveComponent.prototype.deleteStateValueActual = function (stateName, metadata, cb) {
        return this.performComponentWork(function (bytecode, mana, done) {
     ...
     * @param {function} cb
     */
    ActiveComponent.prototype.zShiftIndices = function (componentId, timelineName, timelineTime, newIndex, metadata, cb) {
        var _this = this;
        return this.project.updateHook('zShiftIndices', this.getRelpath(), componentId, timelineName, timelineTime, newIndex, metadata, function (fire) {
            return _this.zShiftIndicesActual(componentId, timelineName, timelineTime, newIndex, metadata, function (err, stackingInfo) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
    ...
    ActiveComponent.prototype.zShiftIndicesImpl = function (bytecode, componentId, timelineName, timelineTime, newIndex) {
        var stackingInfo = Template.getStackingInfo(bytecode, bytecode.template, timelineName, timelineTime);
    ...
    ActiveComponent.prototype.zShiftIndicesActual = function (componentId, timelineName, timelineTime, newIndex, metadata, cb) {
        var _this = this;
        var stackingInfo;
        return this.performComponentTimelinesWork(function (bytecode, mana, timelines, done) {
            stackingInfo = _this.zShiftIndicesImpl(bytecode, componentId, timelineName, timelineTime, newIndex);
            done();
        }, function (err) {
            cb(err, stackingInfo);
        });
     ...
     * @method zMoveToFront
     */
    ActiveComponent.prototype.zMoveToFront = function (componentId, timelineName, timelineTime, metadata, cb) {
        var _this = this;
        return this.project.updateHook('zMoveToFront', this.getRelpath(), componentId, timelineName, timelineTime, metadata, function (fire) {
            return _this.zMoveToFrontActual(componentId, timelineName, timelineTime, metadata, function (err, stackingInfo) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
    ...
    ActiveComponent.prototype.zMoveToFrontImpl = function (bytecode, componentId, timelineName, timelineTime) {
        var stackingInfo = Template.getStackingInfo(bytecode, bytecode.template, timelineName, timelineTime);
        this.grabStackObjectFromStackingInfo(stackingInfo, componentId);
        stackingInfo.push({
            haikuId: componentId,
    ...
    ActiveComponent.prototype.zMoveToFrontActual = function (componentId, timelineName, timelineTime, metadata, cb) {
        var _this = this;
        var stackingInfo;
        return this.performComponentTimelinesWork(function (bytecode, mana, timelines, done) {
            stackingInfo = _this.zMoveToFrontImpl(bytecode, componentId, timelineName, timelineTime);
            done();
        }, function (err) {
            cb(err, stackingInfo);
        });
     ...
     * @method zMoveForward
     */
    ActiveComponent.prototype.zMoveForward = function (componentId, timelineName, timelineTime, metadata, cb) {
        var _this = this;
        return this.project.updateHook('zMoveForward', this.getRelpath(), componentId, timelineName, timelineTime, metadata, function (fire) {
            return _this.zMoveForwardActual(componentId, timelineName, timelineTime, metadata, function (err, stackingInfo) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
    ...
    ActiveComponent.prototype.zMoveForwardActual = function (componentId, timelineName, timelineTime, metadata, cb) {
        var _this = this;
        var stackingInfo;
        return this.performComponentTimelinesWork(function (bytecode, mana, timelines, done) {
            stackingInfo = Template.getStackingInfo(bytecode, mana, timelineName, timelineTime);
            var stackObject = _this.grabStackObjectFromStackingInfo(stackingInfo, componentId);
            var ourStackObject = stackObject && stackObject.ourStackObject;
     ...
     */
    ActiveComponent.prototype.zMoveBackward = function (componentId, timelineName, timelineTime, metadata, cb) {
        var _this = this;
        return this.project.updateHook('zMoveBackward', this.getRelpath(), componentId, timelineName, timelineTime, metadata, function (fire) {
            return _this.zMoveBackwardActual(componentId, timelineName, timelineTime, metadata, function (err, stackingInfo) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
    ...
    ActiveComponent.prototype.zMoveBackwardActual = function (componentId, timelineName, timelineTime, metadata, cb) {
        var _this = this;
        var stackingInfo;
        return this.performComponentTimelinesWork(function (bytecode, mana, timelines, done) {
            stackingInfo = Template.getStackingInfo(bytecode, mana, timelineName, timelineTime);
     ...
     * @method zMoveToBack
     */
    ActiveComponent.prototype.zMoveToBack = function (componentId, timelineName, timelineTime, metadata, cb) {
        var _this = this;
        return this.project.updateHook('zMoveToBack', this.getRelpath(), componentId, timelineName, timelineTime, metadata, function (fire) {
            return _this.zMoveToBackActual(componentId, timelineName, timelineTime, metadata, function (err, stackingInfo) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
    ...
    ActiveComponent.prototype.zMoveToBackActual = function (componentId, timelineName, timelineTime, metadata, cb) {
        var _this = this;
        var stackingInfo;
        return this.performComponentTimelinesWork(function (bytecode, mana, timelines, done) {
            stackingInfo = Template.getStackingInfo(bytecode, mana, timelineName, timelineTime);
     ...
     */
    ActiveComponent.prototype.createTimeline = function (timelineName, timelineDescriptorSerial, metadata, cb) {
        var _this = this;
        var timelineDescriptor = Bytecode.unserializeValue(timelineDescriptorSerial, function (ref) {
            return _this.evaluateReference(ref);
        });
        return this.project.updateHook('createTimeline', this.getRelpath(), timelineName, Bytecode.serializeValue(timelineDescriptor), metadata, function (fire) {
            return _this.createTimelineActual(timelineName, timelineDescriptor, metadata, function (err) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
    ...
    ActiveComponent.prototype.createTimelineActual = function (timelineName, timelineDescriptor, metadata, cb) {
        return this.performComponentWork(function (bytecode, mana, done) {
    ...
    ActiveComponent.prototype.renameTimeline = function (timelineNameOld, timelineNameNew, metadata, cb) {
        var _this = this;
        return this.project.updateHook('renameTimeline', this.getRelpath(), timelineNameOld, timelineNameNew, metadata, function (fire) {
            return _this.renameTimelineActual(timelineNameOld, timelineNameNew, metadata, function (err) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
    ...
    ActiveComponent.prototype.renameTimelineActual = function (timelineNameOld, timelineNameNew, metadata, cb) {
        return this.performComponentWork(function (bytecode, mana, done) {
    ...
    ActiveComponent.prototype.deleteTimeline = function (timelineName, metadata, cb) {
        var _this = this;
        return this.project.updateHook('deleteTimeline', this.getRelpath(), timelineName, metadata, function (fire) {
            return _this.deleteTimelineActual(timelineName, metadata, function (err) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
    ...
    ActiveComponent.prototype.deleteTimelineActual = function (timelineName, metadata, cb) {
        return this.performComponentWork(function (bytecode, mana, done) {
    ...
    ActiveComponent.prototype.duplicateTimeline = function (timelineName, metadata, cb) {
        var _this = this;
        return this.project.updateHook('duplicateTimeline', this.getRelpath(), timelineName, metadata, function (fire) {
            return _this.duplicateTimelineActual(timelineName, metadata, function (err) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
    ...
    ActiveComponent.prototype.duplicateTimelineActual = function (timelineName, metadata, cb) {
        return this.performComponentWork(function (bytecode, mana, done) {
    ...
    ActiveComponent.prototype.changePlaybackSpeed = function (framesPerSecond, metadata, cb) {
        var _this = this;
        return this.project.updateHook('changePlaybackSpeed', this.getRelpath(), framesPerSecond, metadata, function (fire) {
            return _this.changePlaybackSpeedActual(framesPerSecond, metadata, function (err) {
                if (err) {
                    logger.error("[active component (".concat(_this.project.getAlias(), ")]"), err);
                    return cb(err);
                }
    ...
    ActiveComponent.prototype.changePlaybackSpeedActual = function (framesPerSecond, metadata, cb) {
        return this.performComponentWork(function (bytecode, mana, done) {
    ...
    ActiveComponent.prototype.dump = function () {
        ...
        return "".concat(relpath, "(").concat(this.getMount().getRenderId(), ")@").concat(aid, "/").concat(this.interactionMode);
    };
    ...
    ActiveComponent.prototype.syncCode = function (currentEditorContents, metadata, cb) {
        var _this = this;
        ...
        return Lock.request(Lock.LOCKS.FileReadWrite(absPath), false, function (release) {
            return _this.project.updateHook('syncCode', _this.getRelpath(), currentEditorContents, metadata, function (fire) {
                try {
                    var bytecode = ModuleWrapper.testLoadBytecode(currentEditorContents, absPath);
                    _this.fetchActiveBytecodeFile().updateContents(currentEditorContents);
                    _this.handleUpdatedBytecode(bytecode);
                }
                catch (requireError) {
                    release();

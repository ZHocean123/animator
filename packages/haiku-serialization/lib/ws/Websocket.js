...
function Websocket(url, folder, clientType, clientAlias, WebSocket, token) {
    ...
    this.workers = {
        connection: setInterval(function () {
            if (_this._isPermanentlyDisconnected) {
                return null;
            }
            if (_this.ws.readyState === STATES.CLOSING || _this.ws.readyState === STATES.CLOSED) {
                _this.connect();
            }
        }, 1000),
    };
...
Websocket.prototype.setupSocket = function setupSocket() {
    ...
    this.ws.onopen = function () {
        logger.info('[websocket] connection opened (' + _this.url + ')');
        _this.emit('open');
    };
    this.ws.onclose = function () {
        logger.info('[websocket] connection closed (' + _this.url + ')');
        // Sometimes it seems this change happens on a delay so we set it right away
        _this.ws.readyState = _this.WebSocket.CLOSED;
        _this.emit('close');
    };
    ...
    this.ws.onmessage = function (event) {
        ...
        if (message.type === 'signal') {
            return _this.emit('signal', message);
        }
        if (message.type === 'relay') {
            return _this.emit('relay', message);
        }
        // Assume the message is a reply if the incoming has an id that matches one of our outgoing
        if (_this.requests[message.id]) {
            var entry = _this.requests[message.id];
            delete _this.requests[message.id]; // Remove from incoming requests
        ...
        if (typeof message.method === 'string') {
            return _this.emit('method', message.method, message.params || [], message, function (error, result) {
                return _this.sendWhenConnected({
                    id: message.id,
                    folder: message.folder || _this.folder,
...
Websocket.prototype.sendWhenConnected = function sendWhenConnected(message) {
    ...
    return this.whenConnected(function () {
        return _this.sendImmediate(message);
    });
};
...
Websocket.prototype.request = function request(message, callback) {
        ...
        timeoutInstance = setTimeout(function () {
            ...
            if (!gotResponse) {
                ...
                if (typeof message.retry === 'number' && message.retry > 0) {
                    message.retry -= 1;
                    return _this.request(message, callback);
                }
    ...
    this.requests[message.id] = {
        callback: function (err, a, b, c, d, e, f) {
            gotResponse = true;

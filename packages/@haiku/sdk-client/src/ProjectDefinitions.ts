...
export function getHaikuCoreVersion() {
...
export function getHaikuComponentInitialVersion() {
  return FALLBACK_SEMVER_VERSION
}

export function getSafeProjectName(name: string) {
  return (name && name.replace(WHITESPACE_REGEX, UNDERSCORE)) || FALLBACK_PROJECT_NAME
}
...
export function getAngularSelectorName(name: string) {
  return getSafeProjectName(name)
    .replace(/([A-Z])/g, (char: string) => `-${char.toLowerCase()}`)
    .replace(/^-/, '')
}

export function getStandaloneName(organizationName: string, name: string) {
  return `HaikuComponent_${organizationName}_${getProjectNameSafeShort(name)}`
}

export function getCopyrightNotice(organizationName: string) {
  return dedent`
${`Copyright (c) ${(new Date()).getFullYear()} ${organizationName}. All rights reserved.`}
`
}

export function getOrganizationNameOrFallback(organizationName: string) {
...
export function readPackageJson(folder: string) {
...
export function fetchProjectConfigInfo(folder: string, cb: any) {
...
export function storeConfigValues(folder: string, incoming: any, extra = {}) {

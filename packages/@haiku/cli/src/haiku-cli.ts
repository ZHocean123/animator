...
const cli: Nib = new Nib({
  ...
  version: pkg.version,
  description:
    "The Haiku CLI — developer utilities for automating Haiku actions and performing local and" +
  ...
  commands: [
      ...
      flags: [
        {
          ...
          description:
            "include to list organizations your account is a member of instead of projects"
        }
      ],
    ...
    },
    {
      ...
      action: doClone,
      description:
        "Clone a Haiku project to your filesystem, passing through to git clone",
      args: [
        {
          name: "project-name",
          required: true,
        ...
        },
        {
          ...
          required: false,
          usage:
            "Optional: location on the file system where the project should be cloned"
        }
      ]
    },
    {
      ...
      action: doDelete,
      description:
        "Deletes a Haiku project for your entire team.  Cannot be undone.",
      args: [
        {
          name: "project-name",
          required: false,
          usage:
            "Specifies the name of the project to delete (case-sensitive.)  If this isn't provided, the action" +
            " will be interactive."
        }
      ]
    },
    {
      ...
      action: doInit,
      description:
        "Inits a project for installing @haiku modules. " +
    ...
    },
    {
      ...
      action: doInstall,
      description:
        "Install a Haiku project as an npm module, requires a package.json",
      args: [
        {
          name: "project-name",
          required: true,
          usage:
            "Specifies the name of the project to install as a dependency.  Case-sensitive."
        }
      ]
    },
    {
      ...
      args: [
        {
          ...
          required: false,
          usage:
            "Specifies the name of the project to update as a dependency.  Case-sensitive.  If not provided," +
        ...
        },
        {
          ...
          required: false,
          usage:
            "Specifies the version to update specified dependency to.  If not provided, will update to the" +
      ...
      args: [
        {
          ...
          required: true,
          usage:
            "Specifies the name of new component to be generated.  Case-sensitive and must be unique."
        }
      ],
  ...
  ensureAuth(context, token => {
    inquirer
      ...
      .then((answers: inquirer.Answers) => {
        ...
        inkstone.user.changePassword(
          token,
          params,
          (err, responseBody, response) => {
            if (err) {
              context.writeLine(
                chalk.bold("Unable to change password: ") + err
              );
              process.exit(1);
            } else {
...
function doClone(context: IContext) {
  ...
  ensureAuth(context, token => {
    ...
    inkstone.project.get(
      { Name: projectName },
      (getByNameErr, projectAndCredentials) => {
        if (getByNameErr) {
          switch (getByNameErr.message) {
            case ErrorCode.ErrorCodeProjectNotFound:
              context.writeLine(
                chalk.bold(`Project ${projectName} not found.`)
              );
              break;
        ...
        client.git.cloneRepo(
          projectAndCredentials.RepositoryUrl,
          destination,
          cloneErr => {
            if (cloneErr) {
              context.writeLine(
                chalk.red(
                  "Error cloning project.  Use the --verbose flag for more information."
                )
              );
              process.exit(1);
            } else {
              context.writeLine(
                `Project ${chalk.bold(projectName)} cloned to ${chalk.bold(
                  destination
                )}`
              );
              process.exit(0);
...
function doDelete(context: IContext) {
  ensureAuth(context, (token: string) => {
    context.writeLine(
      chalk.bold(
        "Please note that deleting this project will delete it for your entire team."
      )
    );
    ...
    const actuallyDelete = (finalProjectName: string) => {
      inkstone.project.deleteByName({ Name: finalProjectName }, err => {
        if (err) {
          context.writeLine(
            chalk.red("Error deleting project.  Does this project exist?")
          );
          process.exit(1);
        } else {
    ...
    hasbin("npm", (result: boolean) => {
      if (result) {
        ...
        if (fs.existsSync(process.cwd() + "/package.json")) {
              ...
              (getByNameErr, projectAndCredentials) => {
                ...
                try {
                  ...
                  context.writeLine(
                    `${chalk.red(
                      "npm install failed."
                    )} Your Haiku packages have been injected` +
                ...
                context.writeLine(
                  chalk.green("Haiku project installed successfully.")
                );
                process.exit(0);
              }
            );
          ...
          context.writeLine(
            chalk.red(
              "haiku install can only be used at the root of a project with a package.json."
            )
          );
          context.writeLine(
            "You can use " +
              chalk.bold("haiku clone ProjectName [/Optional/Destination]") +
        ...
        context.writeLine(
          chalk.red("npm was not found on this machine. ") +
    ...
    if (context.flags.organizations) {
      ...
      inkstone.project.list((err, projects) => {
        if (!projects || projects.length === 0) {
          context.writeLine(
            "No existing projects.  Use " +
              chalk.bold("haiku generate") +
              " to make a new one!"
          );
          process.exit(0);
        } else {
          ...
          context.writeLine(
            "(To work with one, call " +
              chalk.bold("haiku clone project_name") +
              " or " +
...
function doLogin(context: IContext, cb?: () => void) {
  ...
  inquirer
    ...
    .then((answers: inquirer.Answers) => {
      ...
      inkstone.user.authenticate(
        username,
        password,
        (err, authResponse, httpResponse) => {
          if (err !== undefined) {
            if (httpResponse && httpResponse.statusCode === 403) {
              ...
              context.writeLine(
                chalk.bold.red("Username or password incorrect.")
              );
            }

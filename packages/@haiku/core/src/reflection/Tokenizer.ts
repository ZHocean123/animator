...
export function tokenize(source: string, regexps: any): any {
  const tokens = []
  let chunk = source
...
const PARAMETERS_REGEXPS = [
  ...
  { type: 'colon', re: /^:/ },
  { type: 'comma', re: /^,/ },
  { type: 'identifier', re: /^[\w$]+/ }, // TODO: Include unicode chars
]

export function tokenizeParameters(source: string) {
...
const DIRECTIVE_REGEXPS = [
  ...
  { type: 'identifier', re: new RegExp('^([$A-Z_\\x7f-\\uffff][$\\w\\x7f-\\uffff]*)([^\\n\\S]*:(?!:))?', 'i') },
]

export function tokenizeDirective(source: string) {

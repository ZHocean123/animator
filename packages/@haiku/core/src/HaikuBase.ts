...
function upsertInstanceRegistry(className: string) {
...
function getInstanceRegistry(instance: HaikuBase): HaikuBase[] {
  const className = instance.getClassName()
  return upsertInstanceRegistry(className)
}

function addInstanceToGlobalModelRegistry(instance: HaikuBase): number {
  getInstanceRegistry(instance).push(instance)
  return ++HaikuGlobal.idCounter
}

function removeInstanceFromGlobalModelRegistry(instance: HaikuBase): number {

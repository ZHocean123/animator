...
export function parseAttrSelector(selector) {
...
export function cssMatchByAttribute(node, attrKeyToMatch, attrOperator, attrValueToMatch, options) {
...
export function cssMatchByClass(node, className, options) {
  const attributes = node[options.attributes]
  ...
  if (attributes) {
    ...
    if (foundClassName) {
      ...
      if (classPieces.includes(className)) {
        return true
      }
    }
  }
...
export function cssMatchByHaiku(node, haikuString, options) {
  const attributes = node[options.attributes]
...
export function cssMatchById(node, id, options) {
  const attributes = node[options.attributes]
...
export function cssMatchByTagName(node, tagName, options) {
...
export function cssMatchOne(node, piece, options) {
...
export function cssQueryList(list, query, options) {
  const matches = []
...
export function cssQueryTree(node, query, options) {
...
export function manaFlattenTree(node, options, unique = true, list = [], depth = 0, index = 0) {
  list.push(node)
...
export function visitManaTree(locator: string, mana: string | BytecodeNode, visitor: ManaTreeVisitor, parent: MaybeBytecodeNode, index: number): void {
...
export function visit(mana: BytecodeNode, visitor: (node: BytecodeNode, parent?: BytecodeNode) => void, parent?: BytecodeNode): void {
...
export function ascend(mana, ascender): void {
  if (!mana || typeof mana !== 'object') {
    return
  }
...
export function cloneNodeShallow(node) {
  if (!node) {
    return node
  }

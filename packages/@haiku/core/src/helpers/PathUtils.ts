...
import type { CurveDef, CurveSpec } from '../vendor/svg-points/types'
...
export function splitSegmentInSVGPoints(points: CurveSpec[], pt1Index: number, pt2Index: number, t: number): void {
...
export function cubicBezierSplit(t: number, anchor1: Vec2, handle1: Vec2, handle2: Vec2, anchor2: Vec2): [[Vec2, Vec2, Vec2, Vec2], [Vec2, Vec2, Vec2, Vec2]] {
...
export function mat4_multiply_vec4(m: number[], v: Vec4): Vec4 {
  return {
...
export function polygonLength(points: Vec2[]): number {
  let perimeter = 0
  let a: Vec2
  let b: Vec2 = points[points.length - 1]
  for (let i = 0; i < points.length; i++) {
...
export function distributeTotalVertices(path: CurveSpec[], totalVertices: number): void {
...
export function rotatePathForSmallestDistance(source: CurveSpec[], dest: CurveSpec[]): void {
...
export function ensurePathClockwise(path: CurveSpec[]): void {
...
export function normalizePointCurves(path: CurveSpec[]): void {
...
export function isMultiShape(path: CurveSpec[]): boolean {
  for (let i = 0; i < path.length; i++) {
...
export function synchronizePathStructure(...paths: CurveSpec[][]) {

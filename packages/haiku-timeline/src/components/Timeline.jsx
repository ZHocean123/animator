...
class Timeline extends React.Component {
  constructor(props) {
    ...
    this.handleRequestElementCoordinates = this.handleRequestElementCoordinates.bind(
      this
    );
    ...
    this.handleUndoDebounced = lodash.debounce(
      this.handleUndo.bind(this),
      MENU_ACTION_DEBOUNCE_TIME,
      { leading: true, trailing: false }
    );
    this.handleRedoDebounced = lodash.debounce(
      this.handleRedo.bind(this),
      MENU_ACTION_DEBOUNCE_TIME,
      { leading: true, trailing: false }
    );
    this.handleZoomThrottled = lodash.throttle(
  ...
  componentWillUnmount() {
    ...
    if (this.tourClient) {
      this.tourClient.off(
        ...
        this.handleRequestElementCoordinates
      );
    }
  ...
  getKeyframeCoordinates = keyframe => {
    const keyframeViewEl = document.getElementById(
      `keyframe-container-${keyframe.getUniqueKey()}`
    );
    return keyframeViewEl
      ? keyframeViewEl.firstChild.getBoundingClientRect()
      : null;
  };
    ...
    this.addEmitterListenerIfNotAlreadyRegistered(
      ...
      (what, ...args) => {
        switch (what) {
          ...
          case "zMoveForward":
          case "zMoveBackward":
            const row = Row.findByComponentAndHaikuId(
              this.getActiveComponent(),
              args[1]
            );
    ...
    this.addEmitterListener(
      ...
      lodash.throttle(() => {
        if (this.mounted && this.getActiveComponent()) {
          ...
          this.getActiveComponent()
            .getCurrentTimeline()
            .setTimelinePixelWidth(pxWidth + 20);
          this.forceUpdate();
        }
      }, THROTTLE_TIME)
    );
    ...
    this.addEmitterListener(this.props.websocket, "relay", message => {
      ...
      switch (message.name) {
        ...
        case "global-menu:paste":
          ...
          if (document.hasFocus()) {
            if (
              ...
              !this.refs.expressionInput.willHandlePasteEvent()
            ) {
              this.props.websocket.send(relayable);
            }
          } else {
        ...
        case "global-menu:undo":
          if (window.isWebview) {
            // Let work in standalone dev mode
  ...
  handleActiveComponentReady() {
    ...
    timeline.setTimelinePixelWidth(
      document.body.clientWidth - timeline.getPropertiesPixelWidth() + 20
    );
  ...
  scrollToRow = lodash.throttle(row => {
    const rowElement = document.getElementById(
      `component-heading-row-${row.element.getComponentId()}-${row.getAddress()}`
    );
  ...
  canHaveKeyframes(type, model) {
    if (type === "cluster-row" && model.children && model.children.length > 0) {
      return Property.canHaveKeyframes(
        model.children[0].property.name,
        model.element
      );
    }
  ...
  getPopoverMenuItems({ event, type, model, offset, curve }) {
    ...
    const isTweenableTransitionSegment =
      type === "keyframe-segment" && model && model.isTweenable();
    ...
    items.push({
      ...
      enabled:
        this.getActiveComponent().checkIfSelectedKeyframesAreMovableToZero() &&
        canHaveKeyframes,
      onClick: () => {
    ...
    items.push({
      ...
      submenu:
        isTweenableTransitionSegment &&
        this.curvesMenu(curve, (_, curveName) => {
          this.getActiveComponent().joinSelectedKeyframes(curveName, {
            from: "timeline"
          });
        })
    });
    ...
    items.push({
      ...
      submenu:
        type === "keyframe-transition" &&
        this.curvesMenu(curve, (_, curveName) => {
          this.getActiveComponent().changeCurveOnSelectedKeyframes(curveName, {
            from: "timeline"
          });
        })
    });
  ...
  handleExportProgress(exporterRequest) {
    ...
    const activeRequestIndex = trackedExporterRequests.findIndex(
      trackedExporterRequest =>
        trackedExporterRequest.filename === exporterRequest.filename
    );
    if (activeRequestIndex !== -1) {
      if (exporterRequest.progress === 0) {
        ...
        trackedExporterRequests[activeRequestIndex].progress =
          exporterRequest.progress;
      }
  ...
  trackExportProgress() {
    this.project
      .getEnvoyClient()
      .get(EXPORTER_CHANNEL)
      .then(exporterChannel => {
    ...
    if (!this.project.getEnvoyClient().isInMockMode()) {
      this.project
        ...
        .then(user => {
          this.user = user;
          user
            .getConfig(UserSettings.TimeDisplayModes)
            .then(timeDisplayModes => {
              if (
                timeDisplayModes &&
                timeDisplayModes[this.project.getFolder()]
              ) {
                this.getActiveComponent()
                  .getCurrentTimeline()
                  .setTimeDisplayMode(
                ...
                user
                  .getConfig(UserSettings.DefaultTimeDisplayMode)
                  .then(defaultTimeDisplayMode => {
                    if (defaultTimeDisplayMode) {
                      this.getActiveComponent()
                        .getCurrentTimeline()
                        .setTimeDisplayMode(defaultTimeDisplayMode);
                    }
                  });
              }
            });
  ...
  handleZoom(wheelEvent) {
    const maxZoom = 80;
    const delta =
      Math.abs(wheelEvent.deltaY) > maxZoom
        ? Math.sign(wheelEvent.deltaY) * maxZoom
        : wheelEvent.deltaY;
    this.getActiveComponent()
      .getCurrentTimeline()
      .zoomBy(delta * 0.01);
  ...
  handleHorizontalScroll = () => {
    this.getActiveComponent()
      .getCurrentTimeline()
      .setScrollLeftFromScrollbar(this.container.scrollLeft);
  };
  ...
  handleKeyDown(nativeEvent) {
    ...
    const willExprInputHandle = this.refs.expressionInput.willHandleExternalKeydownEvent(
      nativeEvent
    );
    ...
    switch (nativeEvent.which) {
      ...
      case 37: // left
        if (this.isCommandKeyDown) {
          if (this.isShiftKeyDown) {
            ...
            this.getActiveComponent()
              .getCurrentTimeline()
              .updateScrubberPositionByDelta(-1);
          }
        } else {
          nativeEvent.preventDefault();
      ...
      // case 8: //delete
      case 16:
        this.updateKeyboardState({ isShiftKeyDown: true });
        break;
      case 17:
        this.updateKeyboardState({ isControlKeyDown: true });
        break;
      case 18:
        this.updateKeyboardState({ isAltKeyDown: true });
        break;
      case 224:
        this.updateKeyboardState({ isCommandKeyDown: true });
        break;
      case 91:
        this.updateKeyboardState({ isCommandKeyDown: true });
        break;
  ...
  handleKeyUp(nativeEvent) {
    switch (nativeEvent.which) {
      ...
      case 46:
        this.getActiveComponent().deleteSelectedKeyframes({ from: "timeline" });
        break; // Only if there are any
      case 8:
        this.getActiveComponent().deleteSelectedKeyframes({ from: "timeline" });
        break; // Only if there are any
      case 16:
        this.updateKeyboardState({ isShiftKeyDown: false });
        break;
      case 17:
        this.updateKeyboardState({ isControlKeyDown: false });
        break;
      case 18:
        this.updateKeyboardState({ isAltKeyDown: false });
        break;
      case 224:
        this.updateKeyboardState({ isCommandKeyDown: false });
        break;
      case 91:
        this.updateKeyboardState({ isCommandKeyDown: false });
        break;
  ...
  saveTimeDisplayModeSetting = () => {
    const mode = this.getActiveComponent()
      .getCurrentTimeline()
      .getTimeDisplayMode();
    ...
    if (!this.project.getEnvoyClient().isInMockMode()) {
      this.user
        .getConfig(UserSettings.TimeDisplayModes)
        .then(timeDisplayModes => {
          this.user.setConfig(UserSettings.TimeDisplayModes, {
            ...timeDisplayModes,
      ...
      >
        <ControlsArea
          ...
          changeTimelineName={(oldTimelineName, newTimelineName) => {
            this.getActiveComponent().renameTimeline(
              oldTimelineName,
              newTimelineName,
              { from: "timeline" },
              () => { }
          ...
          createTimeline={timelineName => {
            this.getActiveComponent().createTimeline(
              timelineName,
              {},
              { from: "timeline" },
              () => { }
        ...
        >
          {experimentIsEnabled(Experiment.LocalAssetExport) && (
            <TrackedExporterRequests
              trackedExporterRequests={this.state.trackedExporterRequests}
            />
          )}
        </div>
      </div>
  ...
  getCurrentTimelineTime(frameInfo) {
    return Math.round(
      this.getActiveComponent()
        .getCurrentTimeline()
        .getCurrentFrame() * frameInfo.mspf
    );
  }

  showFrameActionsEditor(frame) {
    const elementPrimaryKey = this.getActiveComponent()
      .findElementRoot()
      .getPrimaryKey();
    this.showEventHandlersEditor(elementPrimaryKey, frame);
  }
  ...
  renderTopControls() {
    ...
    return [
      ...
      >
        <GaugeTimeReadout
          saveTimeDisplayModeSetting={this.saveTimeDisplayModeSetting}
          timeline={timeline}
        />
      </div>,
      <SimplifiedFrameGrid
  ...
  mouseMoveListener(evt) {
    ...
    const frameInfo = this.getActiveComponent()
      .getCurrentTimeline()
      .getFrameInfo();
    const leftX =
      evt.clientX +
  ...
  setGlassInteractionToEditMode = () => {
    if (this.state.isPreviewModeActive) {
      this.project.setInteractionMode(
  ...
  render() {
    if (
      !this.getActiveComponent() ||
      !this.getActiveComponent().getCurrentTimeline()
    ) {
      return (
        <div
          id="timeline"
      ...
      >
        ...
        <ExpressionInput
          ...
          onNavigateRequested={(navDir, doFocus) => {
            activeComponent.focusSelectNext(navDir, doFocus, {
              from: "timeline"
            });
          }}
        />

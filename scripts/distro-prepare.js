...
const PNPM_INSTALL_FLAGS = [
  '--frozen-lockfile', // Force use of dependencies from pnpm-lock.yaml
  '--prefer-offline', // Use offline cache when possible
...
logExec(ROOT, `pnpm install ${PNPM_INSTALL_FLAGS} --prod=false`);
logExec(ROOT, `pnpm compile-all --force`);
logExec(ROOT, `pnpm install ${PNPM_INSTALL_FLAGS} --prod`);
